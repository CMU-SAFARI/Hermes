...
string map_to_string(const vector<MLOP_State> &access_map,
                     const vector<int>        &prefetch_map)
{
  ostringstream oss;
...
MLOP::MLOP(string type, CACHE *cache) : Prefetcher(type), parent(cache)
{
  ...
  access_map_table =
      new AccessMapTable(amt_size, blocks_in_zone, PF_DEGREE - 1, debug_level);
...
void MLOP::access(uint64_t block_number)
{
  if (this->debug_level >= 2) {
    cout << "MLOP::access(block_number=0x" << hex << block_number << ")" << dec
         << endl;
  }
  ...
  if (this->debug_level >= 2) {
    cout << "[MLOP::access] zone_number=0x" << hex << zone_number << dec
         << ", zone_offset=" << zone_offset << endl;
  }
  ...
  if (!entry) {
    ...
    if (this->zone_cnt == TRACKED_ZONE_CNT) {
      ...
      this->tracking            = true;
      this->zone_life.push_back(
          string(this->blocks_in_zone, state_char[MLOP_State::INIT]));
    }
    /* ===== */
    return;
  }
  ...
  if (this->update_cnt == NUM_UPDATES) {
    ...
    for (uint32_t i = 0; i < PF_DEGREE; i += 1) {
      max_scores[i] = *max_element(this->offset_scores[i].begin(),
    ...
    /* reset `offset_scores` */
    this->offset_scores =
        vector<vector<int>>(PF_DEGREE, vector<int>(NUM_OFFSETS, 0));
...
{
  if (this->debug_level >= 2) {
    cout << "MLOP::prefetch(cache=" << cache->NAME << "-" << cache->cpu
         << ", block_number=0x" << hex << block_number << dec << ")" << endl;
  }
  int                    pf_issued   = 0;
  uint64_t               zone_number = block_number / this->blocks_in_zone;
  int                    zone_offset = block_number % this->blocks_in_zone;
  ...
  if (this->debug_level >= 2) {
    cout << "[MLOP::prefetch] old_access_map="
         << map_to_string(access_map, prefetch_map) << endl;
  }
  for (uint32_t d = 0; d < PF_DEGREE; d += 1) {
  ...
  if (this->debug_level >= 2) {
    cout << "[MLOP::prefetch] new_access_map="
         << map_to_string(access_map, prefetch_map) << endl;
...
string MLOP::log_offset_scores()
{
  ...
  for (uint32_t i = -(this->blocks_in_zone - 1);
       i <= +(this->blocks_in_zone - 1); i += 1) {
    table.set_cell(i + this->blocks_in_zone, 0, (int)i);
    for (uint32_t d = 0; d < PF_DEGREE; d += 1) {
      table.set_cell(i + this->blocks_in_zone, d + 1,
                     (int)this->offset_scores[d][ORIGIN + i]);
    }
  }
...
void MLOP::print_stats()
{
  ...
  double max_score_le_sqr_mean =
      1.0 * this->max_score_le_sqr_sum / this->round_cnt;
  double max_score_le_sd =
      sqrt(max_score_le_sqr_mean - square(max_score_le_mean));
  cout << "[MLOP] Max Score Left Mean (%): "
       << 100.0 * max_score_le_mean / NUM_UPDATES << endl;
  cout << "[MLOP] Max Score Left SD (%): "
       << 100.0 * max_score_le_sd / NUM_UPDATES << endl;

  double max_score_ri_mean = 1.0 * this->max_score_ri_sum / this->round_cnt;
  double max_score_ri_sqr_mean =
      1.0 * this->max_score_ri_sqr_sum / this->round_cnt;
  double max_score_ri_sd =
      sqrt(max_score_ri_sqr_mean - square(max_score_ri_mean));
  cout << "[MLOP] Max Score Right Mean (%): "
       << 100.0 * max_score_ri_mean / NUM_UPDATES << endl;
  cout << "[MLOP] Max Score Right SD (%): "
       << 100.0 * max_score_ri_sd / NUM_UPDATES << endl;
  cout << endl;
}
...
void MLOP::register_fill(uint64_t addr, uint32_t set, uint32_t way,
