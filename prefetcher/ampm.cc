...
void AMPM::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit,
                             uint8_t type, vector<uint64_t> &pref_addr)
{
  ...
  auto           it       = find_if(
      ...
      [page](AMPM_PB_Entry *pb_entry) { return pb_entry->page_id == page; });
  ...
  for (uint32_t index = 0; index < selected_pos.size(); ++index) {
    ...
    assert(abs(selected_pos[index]) >= 1 &&
           abs(selected_pos[index]) <= knob::ampm_max_delta);
  ...
  for (uint32_t index = 0; index < selected_neg.size(); ++index) {
    ...
    assert(abs(selected_neg[index]) >= 1 &&
           abs(selected_neg[index]) <= knob::ampm_max_delta);
...
void AMPM::dump_stats()
{
  cout << "ampm.invoke_called " << stats.invoke_called << endl
       ...
       << "ampm.pred.degree_reached_pos " << stats.pred.degree_reached_pos
       << endl
       << "ampm.pred.degree_reached_neg " << stats.pred.degree_reached_neg
       << endl
       << endl;
  ...
  for (uint32_t index = 0; index < MAX_OFFSETS; ++index) {
    if (stats.pred.pos_histogram[index]) {
      cout << "ampm.pred.histogram." << index << " "
           << stats.pred.pos_histogram[index] << endl;
    }
  }
  for (uint32_t index = 0; index < MAX_OFFSETS; ++index) {
    if (stats.pred.neg_histogram[index]) {
      cout << "ampm.pred.histogram.-" << index << " "
           << stats.pred.neg_histogram[index] << endl;
    }
  }
  cout << endl;
