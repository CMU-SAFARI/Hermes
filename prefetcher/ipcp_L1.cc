...
IPCP_L1::IPCP_L1(string type, CACHE *cache)
...
void IPCP_L1::check_for_stream_l1(int index, uint64_t cl_addr, uint8_t cpu)
{
  ...
  if (count > NUM_GHB_ENTRIES / 2) {  // stream is detected
    ...
    if (count >=
        (NUM_GHB_ENTRIES * 3) / 4) {  // stream is classified as strong if more
                                      // than 3/4th entries belong to stream
...
void IPCP_L1::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit,
                                uint8_t type, std::vector<uint64_t> &pref_addr)
{
  ...
  if (trackers_l1[cpu][index].str_valid == 1) {  // stream IP
    ...
    for (int i = 0; i < prefetch_degree; i++) {
      ...
      metadata = encode_metadata(trackers_l1[cpu][index].last_stride, CS_TYPE,
                                 spec_nl[cpu]);
      m_parent_cache->prefetch_line(ip, addr, pf_address, FILL_L1, metadata);
      num_prefs++;
    ...
    for (i = 0; i < prefetch_degree; i++) {
      ...
      if (DPT_l1[cpu][signature].conf >
          0) {  // prefetch only when conf>0 for CPLX
        m_parent_cache->prefetch_line(ip, addr, pf_address, FILL_L1, metadata);
        num_prefs++;
...
void IPCP_L1::cache_fill(uint64_t addr, uint32_t set, uint32_t way,
