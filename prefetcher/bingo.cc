...
{
  assert((knob::bingo_region_size >> LOG2_BLOCK_SIZE) ==
         knob::bingo_pattern_len);
}

void Bingo::init_stats() {}

Bingo::Bingo(string type, CACHE *cache)
    : Prefetcher(type), parent(cache), pattern_len(knob::bingo_pattern_len),
      filter_table(knob::bingo_ft_size, knob::bingo_debug_level),
      accumulation_table(knob::bingo_at_size, knob::bingo_pattern_len,
                         knob::bingo_debug_level),
      pht(knob::bingo_pht_size, knob::bingo_pattern_len,
          knob::bingo_min_addr_width, knob::bingo_max_addr_width,
          knob::bingo_pc_width, knob::bingo_debug_level, knob::bingo_pht_ways),
      pf_streamer(knob::bingo_pf_streamer_size, knob::bingo_pattern_len,
                  knob::bingo_debug_level),
      debug_level(knob::bingo_debug_level)
{
  init_knobs();
  init_stats();
...
void Bingo::print_config()
{
  cout << "bingo_region_size " << knob::bingo_region_size << endl
       ...
       << "bingo_pc_address_fill_level " << knob::bingo_pc_address_fill_level
       << endl
       << endl;
}
...
void Bingo::access(uint64_t block_number, uint64_t pc)
{
  if (this->debug_level >= 2) {
    cerr << "[Bingo] access(block_number=0x" << hex << block_number << ", pc=0x"
         << pc << ")" << dec << endl;
  }
...
void Bingo::invoke_prefetcher(uint64_t pc, uint64_t addr, uint8_t cache_hit,
                              uint8_t type, std::vector<uint64_t> &pref_addr)
{
  if (debug_level >= 2) {
    cerr << "CACHE::l1d_prefetcher_operate(addr=0x" << hex << addr << ", PC=0x"
         << pc << ", cache_hit=" << dec << (int)cache_hit
         << ", type=" << (int)type << ")" << dec << endl;
  }
...
void Bingo::register_fill(uint64_t addr, uint32_t set, uint32_t way,
