                                ...
                                uint32_t action2)
{
  ...
  if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy) {
    ...
    MYLOG("Q(%x,%u) = %.2f, R = %d, Q(%x,%u) = %.2f, Q(%x,%u) = %.2f", state1,
          action1, Qsa1_old, reward, state2, action2, Qsa2, state1, action1,
          Qsa1);
...
void LearningEngineBasic::plot_scores()
{
  ...
  for (uint32_t index = 0; index < knob::le_plot_actions.size(); ++index) {
    ...
    fprintf(script, "'%s' using 1:%u with lines title \"action_index[%u]\"",
            ...
            (knob::le_plot_actions[index] + 2), knob::le_plot_actions[index]);
  }
  fprintf(script, "\n");
  fclose(script);
