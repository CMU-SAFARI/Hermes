...
{
  uint32_t set = spp_ppf::get_hash(page) % ST_SET, match = ST_WAY,
...
{
  // Update (sig, delta) correlation
  uint32_t set = spp_ppf::get_hash(curr_sig) % PT_SET, local_conf = 0,
    ...
    for (uint32_t way = 0; way < PT_WAY; way++) {
      ...
      if (pf_conf && do_pf && pf_q_tail < 100) {
        ...
        SPP_DP(cout << "[PT] " << __func__ << "  LOW CONF: " << pf_conf
               ...
               cout << " delta: " << delta[set][way] << " c_delta: "
                    << c_delta[set][way] << " c_sig: " << c_sig[set];
               cout << " conf: " << local_conf << " pf_q_tail: " << (pf_q_tail)
                    << " depth: " << depth << endl;);
      }
                            ...
                            uint32_t conf, int32_t sum, uint32_t depth)
{
    ...
    if ((valid[quotient] || useful[quotient]) &&
      ...
      SPP_DP(
          ...
          cout << " More Recorded Metadata: Addr: " << hex
               ...
               << confidence_reject[quotient_reject] << endl;);
    }
    break;
  ...
  case L2C_EVICT:
    ...
    if (valid[quotient] && !useful[quotient]) {
      ...
      SPP_DP(cout << "[FILTER] " << __func__ << " eviction for check_addr: "
                  << hex << check_addr << " cache_line: " << cache_line << dec;
             cout << " quotient: " << quotient << " valid: " << valid[quotient]
                  << " useful: " << useful[quotient] << endl;
             cout
             << " Calling Perceptron Update (DEC) as L2C_DEMAND was not useful"
             << endl;
...
void PERCEPTRON::get_perc_index(uint64_t base_addr, uint64_t ip, uint64_t ip_1,
...
int32_t PERCEPTRON::perc_predict(uint64_t base_addr, uint64_t ip, uint64_t ip_1,
                                 ...
                                 uint32_t depth)
{
  SPP_DP(int sig_delta = (cur_delta < 0)
                             ? (((-1) * cur_delta) + (1 << (SIG_DELTA_BIT - 1)))
                             : cur_delta;
  ...
  SPP_DP(
      cout << " Sum of perceptrons: " << sum << " Prediction made: "
           << ((sum >= knob::ppf_perc_threshold_lo)
                   ? ((sum >= knob::ppf_perc_threshold_hi) ? FILL_L2 : FILL_LLC)
                   : 0)
           << endl;);
  // Return the sum
  return sum;
                             ...
                             bool direction, int32_t perc_sum)
{
  ...
  if (!direction) {  // direction = 1 means the sum was in the correct
    ...
    SPP_DP(int differential = (sum >= knob::ppf_perc_threshold_hi) ? -1 : 1;
