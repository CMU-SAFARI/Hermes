/* DISCLAIMER: **NOT** the most-efficient optimization of Sandbox prefetcher,
 * Pugsley+ HPCA 2014 */
...
SandboxPrefetcher::SandboxPrefetcher(string type) : Prefetcher(type)
{
  ...
  /* init bloom filter */
  /* for a given m (bloom filter bit array length) and n (number of elements to
   * be inserted) the k (number of hash functions) to produce minimal false +ve
   * rate would be m/n*ln(2)
   * https://en.wikipedia.org/wiki/Bloom_filter#Probability_of_false_positives
   */
  opt_hash_functions =
      (uint32_t)ceil(knob::sandbox_bloom_filter_size /
                     knob::sandbox_num_access_in_phase * log(2));
  bf = new bf::basic_bloom_filter(
      bf::make_hasher(opt_hash_functions, knob::sandbox_seed, true),
      knob::sandbox_bloom_filter_size);
  assert(bf);
...
void SandboxPrefetcher::print_config()
{
  cout << "sandbox_pref_degree " << knob::sandbox_pref_degree << endl
       << "sandbox_enable_stream_detect " << knob::sandbox_enable_stream_detect
       << endl
       << "sandbox_stream_detect_length " << knob::sandbox_stream_detect_length
       << endl
       << "sandbox_num_access_in_phase " << knob::sandbox_num_access_in_phase
       << endl
       << "sandbox_num_cycle_offsets " << knob::sandbox_num_cycle_offsets
       << endl
       << "sandbox_bloom_filter_size " << knob::sandbox_bloom_filter_size
       << endl
...
void SandboxPrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address,
                                          ...
                                          vector<uint64_t> &pref_addr)
{
  ...
  if (lookup) {
    ...
    if (knob::sandbox_enable_stream_detect) {
      /* RBERA: TODO */
      for (uint32_t index = 1; index <= knob::sandbox_stream_detect_length;
           ++index) {
        int32_t stream_offset =
            offset - (evaluated_offsets[eval.curr_ptr]->offset * index);
        if (stream_offset >= 0 && stream_offset < 64) {
          uint64_t stream_addr =
              (page << LOG2_PAGE_SIZE) + (stream_offset << LOG2_BLOCK_SIZE);
  ...
  if (pref_offset >= 0 && pref_offset < 64) {
    uint64_t pseudo_pref_addr =
        (page << LOG2_PAGE_SIZE) + (pref_offset << LOG2_BLOCK_SIZE);
    filter_add(pseudo_pref_addr);
    stats.step2.filter_add++;
  }

  /* Step 3: check the end of current phase or the end of current round */
  /* Phase end := the current offset has been evaluated for a number of cache
   * accesses Round end := all 16 offsets has been evaluated */
   ...
   * 1. Creates two separate lists of Scores based on posetive and negative
   * offsets
   * 2. Sorts both offset lists based on ABSOLUTE offset value, as Snadbox
   * prefers smaller offsets for prefetching */
...
void SandboxPrefetcher::get_offset_list_sorted(vector<Score *> &pos_offsets,
                                               vector<Score *> &neg_offsets)
{
  for (uint32_t index = 0; index < evaluated_offsets.size(); ++index) {
    ...
    if (evaluated_offsets[index]->offset > 0) {
      pos_offsets.push_back(new Score(evaluated_offsets[index]->offset,
                                      evaluated_offsets[index]->score));
    } else {
      neg_offsets.push_back(new Score(evaluated_offsets[index]->offset,
                                      evaluated_offsets[index]->score));
    }
  }

  std::sort(pos_offsets.begin(), pos_offsets.end(),
            [](Score *score1, Score *score2) {
              return abs(score1->offset) < abs(score2->offset);
            });
  std::sort(neg_offsets.begin(), neg_offsets.end(),
            [](Score *score1, Score *score2) {
              return abs(score1->offset) < abs(score2->offset);
            });
}

void SandboxPrefetcher::generate_prefetch(vector<Score *> offset_list,
                                          uint32_t pref_degree, uint64_t page,
                                          uint32_t          offset,
                                          vector<uint64_t> &pref_addr)
{
  uint32_t count = 0;
  for (uint32_t index = 0; index < offset_list.size(); ++index) {
    for (uint32_t lookahead = 1;
         lookahead <= knob::sandbox_stream_detect_length + 1; ++lookahead) {
      if (lookahead > 1 && !knob::sandbox_enable_stream_detect) {
        break;
      }

      if (offset_list[index]->score >=
          lookahead * knob::sandbox_num_access_in_phase) {
        uint64_t addr = generate_address(page, offset,
                                         offset_list[index]->offset, lookahead);
        if (addr != 0xdeadbeef) {
...
uint64_t SandboxPrefetcher::generate_address(uint64_t page, uint32_t offset,
                                             int32_t delta, uint32_t lookahead)
{
...
void SandboxPrefetcher::dump_stats()
{
  cout << "sandbox_called " << stats.called << endl
       ...
       << "sandbox_step4_pref_generated_pos " << stats.step4.pref_generated_pos
       << endl
       << "sandbox_step4_pref_generated_neg " << stats.step4.pref_generated_neg
       << endl
       << endl;
