...
void POWER7_Pref::print_config()
{
  cout << "streamer_num_trackers " << knob::streamer_num_trackers << endl
       ...
       << "power7_default_streamer_degree "
       << knob::power7_default_streamer_degree << endl
       << endl;
}

POWER7_Pref::POWER7_Pref(string type, CACHE *cache)
...
void POWER7_Pref::invoke_prefetcher(uint64_t pc, uint64_t address,
                                    ...
                                    vector<uint64_t> &pref_addr)
{
  ...
  if (mode == Mode::Explore) {
    for (uint32_t cfg = (uint32_t)Config::Default; config < (uint32_t)config;
         ++cfg) {
...
{
  for (uint32_t index = (uint32_t)Config::Default;
       index < (uint32_t)Config::NumConfigs; ++index) {
...
void POWER7_Pref::dump_stats()
{
  cout << "power7.called " << stats.called << endl
       ...
       << "power7.mode.explore_to_exploit " << stats.mode.explore_to_exploit
       << endl
       << "power7.mode.exploit_to_explore " << stats.mode.exploit_to_explore
       << endl
       << endl;

  for (uint32_t cfg = (uint32_t)Config::Default;
       cfg < (uint32_t)Config::NumConfigs; ++cfg) {
    for (uint32_t mode = (uint32_t)Mode::Explore;
         mode < (uint32_t)Mode::NumModes; ++mode) {
      cout << "power7.config." << get_config_string((Config)cfg) << "."
           << get_mode_string((Mode)mode) << " "
           << stats.config.histogram[cfg][mode] << endl;
    }
  }
  cout << endl;
