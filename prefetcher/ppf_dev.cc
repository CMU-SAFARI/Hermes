...
SPP_PPF_dev::SPP_PPF_dev(std::string type, CACHE *cache)
                                    ...
                                    std::vector<uint64_t> &pref_addr)
{
  ...
  do {
    ...
    // calls perc_predict()
    PT.read_pattern(curr_sig, delta_q, confidence_q, perc_sum_q, lookahead_way,
                    lookahead_conf, pf_q_tail, depth, addr, base_addr,
                    train_addr, curr_ip, train_delta, last_sig,
                    m_parent_cache->PQ.occupancy, m_parent_cache->PQ.SIZE,
                    m_parent_cache->MSHR.occupancy, m_parent_cache->MSHR.SIZE);
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      ...
      SPP_DP(cout << "[ChampSim] State of features: \nTrain addr: "
                  ...
                  << "\t:Depth " << depth << "\tSUM: " << perc_sum << endl;);
      FILTER_REQUEST fill_level = (perc_sum >= knob::ppf_perc_threshold_hi)
                                      ? SPP_L2C_PREFETCH
                                      : SPP_LLC_PREFETCH;
        ...
        if (num_pf < ceil(((m_parent_cache->PQ.SIZE) / distinct_pages))) {
          if (FILTER.check(pf_addr, train_addr, curr_ip, fill_level,
            ...
            m_parent_cache->prefetch_line(
                ip, addr, pf_addr,
                ((fill_level == SPP_L2C_PREFETCH) ? FILL_L2 : FILL_LLC), 5);
            num_pf++;
            ...
            SPP_DP(
                cout << "[ChampSim] " << __func__ << " base_addr: " << hex
...
#ifdef GHR_ON
        // Store this prefetch request in GHR to bootstrap SPP learning when we
        // see a ST miss (i.e., accessing a new page)
