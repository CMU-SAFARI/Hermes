...
void ScoobyRecorder::record_access_knowledge(Scooby_STEntry *stentry)
{
  ...
  uint64_t bmp_real_rot_value =
      BitmapHelper::value(BitmapHelper::rotate_left(bmp_real, trigger_offset));
...
void ScoobyRecorder::dump_stats()
{
  ...
  if (knob::scooby_access_debug) {
    ...
    for (auto itr = access_bitmap_dist.begin(); itr != access_bitmap_dist.end();
         ++itr) {
      pairs.push_back(*itr);
    }
    ...
    sort(pairs.begin(), pairs.end(),
         [](std::pair<uint64_t, uint64_t> &a,
            std::pair<uint64_t, uint64_t> &b) { return a.second > b.second; });
    ...
    for (uint32_t index = 0; index < pairs.size(); ++index) {
      ...
      if (index < 20) {
        ...
        cout << hex << pairs[index].first << dec << "," << pairs[index].second
             << endl;
      }
    }
...
void print_access_debug(Scooby_STEntry *stentry)
{
  ...
  fprintf(stdout, "[ACCESS] %16lx|%16lx|%2u|%2u|%64s|%64s|%2u|%2u|%2u|%2u|%2u|",
          ...
          BitmapHelper::count_bits_diff(stentry->bmp_pred,
                                        stentry->bmp_real) /* over prediction */
  );
