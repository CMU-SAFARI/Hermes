...
SPP_dev2::SPP_dev2(std::string type, CACHE *cache)
...
void SPP_dev2::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit,
                                 uint8_t type, std::vector<uint64_t> &pref_addr)
{
  uint64_t page = addr >> LOG2_PAGE_SIZE;
  uint32_t page_offset =
               (addr >> LOG2_BLOCK_SIZE) & (PAGE_SIZE / BLOCK_SIZE - 1),
           last_sig = 0, curr_sig = 0, confidence_q[L2C_MSHR_SIZE], depth = 0;
  ...
  do {
    ...
    PT.read_pattern(curr_sig, delta_q, confidence_q, lookahead_way,
                    lookahead_conf, pf_q_tail, depth, GHR);
      ...
      if (confidence_q[i] >= knob::spp_dev2_pf_threshold) {
        ...
        if ((addr & ~(PAGE_SIZE - 1)) ==
          ...
          if (FILTER.check(pf_addr,
                           confidence_q[i] >= knob::spp_dev2_fill_threshold
                               ? SPP_L2C_PREFETCH
                               : SPP_LLC_PREFETCH,
                           GHR)) {
            m_parent_cache->prefetch_line(
                ip, addr, pf_addr,
                ((confidence_q[i] >= knob::spp_dev2_fill_threshold) ? FILL_L2
                                                                    : FILL_LLC),
            ...
            SPP_DP(cout << "[ChampSim] " << __func__ << " base_addr: " << hex
                   ...
                   cout << " depth: " << i << " FILL_THRESH: "
                        << knob::spp_dev2_fill_threshold << " fill_level: "
                        << ((confidence_q[i] >= knob::spp_dev2_fill_threshold)
                                ? FILL_L2
                                : FILL_LLC)
                        << endl;);
...
void SPP_dev2::cache_fill(uint64_t addr, uint32_t set, uint32_t way,
...
void SPP_dev2::dump_stats()
{
  ...
  sort(pairs.begin(), pairs.end(),
       [](pair<int32_t, uint64_t> &a, pair<int32_t, uint64_t> &b) {
         return a.second > b.second;
       });
  for (uint32_t index = 0; index < pairs.size(); ++index) {
    cout << "delta_" << pairs[index].first << " " << pairs[index].second
         << endl;
  }
  cout << endl;

  vector<pair<uint32_t, unordered_map<int32_t, uint64_t>>> pairs2;
  for (auto it = depth_delta_histogram.begin();
       it != depth_delta_histogram.end(); ++it) {
    pairs2.push_back(*it);
  }
  sort(pairs2.begin(), pairs2.end(),
       [](pair<uint32_t, unordered_map<int32_t, uint64_t>> &a,
          pair<uint32_t, unordered_map<int32_t, uint64_t>> &b) {
         return a.first < b.first;
       });
  for (uint32_t index = 0; index < pairs2.size(); ++index) {
    vector<pair<int32_t, uint64_t>> pairs3;
    for (auto it = pairs2[index].second.begin();
         it != pairs2[index].second.end(); ++it) {
      pairs3.push_back(*it);
    }
    sort(pairs3.begin(), pairs3.end(),
         [](pair<int32_t, uint64_t> &a, pair<int32_t, uint64_t> &b) {
           return a.second > b.second;
         });

    for (uint32_t index2 = 0; index2 < pairs3.size(); ++index2) {
      cout << "depth_" << pairs2[index].first << "_delta_"
           << pairs3[index2].first << " " << pairs3[index2].second << endl;
    }
  }
  cout << endl;
}
