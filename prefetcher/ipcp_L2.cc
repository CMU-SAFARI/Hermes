...
IPCP_L2::IPCP_L2(string type, CACHE *cache)
...
void IPCP_L2::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit,
                                ...
                                std::vector<uint64_t> &pref_addr)
{
  ...
  if (trackers[cpu][index].ip_tag != ip_tag) {  // new/conflict IP
    if (trackers[cpu][index].ip_valid ==
      ...
      trackers[cpu][index].ip_valid =
          0;  // otherwise, reset valid bit and leave the previous IP as it is
    }
  ...
  if (trackers[cpu][index].stride != 0) {
    if (trackers[cpu][index].pref_type == 0x100 ||
      ...
      for (int i = 0; i < prefetch_degree; i++) {
        uint64_t pf_address =
            (cl_addr + (trackers[cpu][index].stride * (i + 1)))
            << LOG2_BLOCK_SIZE;
...
void IPCP_L2::cache_fill(uint64_t addr, uint32_t set, uint32_t way,
