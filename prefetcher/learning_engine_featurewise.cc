...
{
  assert(knob::le_featurewise_active_features.size() ==
         knob::le_featurewise_num_tilings.size());
  assert(knob::le_featurewise_active_features.size() ==
         knob::le_featurewise_num_tiles.size());
  assert(knob::le_featurewise_active_features.size() ==
         knob::le_featurewise_enable_tiling_offset.size());
  assert(knob::le_featurewise_active_features.size() ==
         knob::le_featurewise_feature_weights.size());
...
LearningEngineFeaturewise::LearningEngineFeaturewise(
    Prefetcher *parent, float alpha, float gamma, float epsilon,
    uint32_t actions, uint64_t seed, std::string policy, std::string type,
    bool zero_init)
    : LearningEngineBase(parent, alpha, gamma, epsilon, actions,
                         0 /*dummy state value*/, seed, policy, type)
{
  /* init each feature engine */
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    m_feature_knowledges[index] = NULL;
  }
  for (uint32_t index = 0; index < knob::le_featurewise_active_features.size();
       ++index) {
    ...
    m_feature_knowledges[knob::le_featurewise_active_features[index]] =
        new FeatureKnowledge(
            (FeatureType)knob::le_featurewise_active_features[index], alpha,
            gamma, actions, knob::le_featurewise_feature_weights[index],
            knob::le_featurewise_weight_gradient,
            knob::le_featurewise_num_tilings[index],
            knob::le_featurewise_num_tiles[index], zero_init,
  ...
  m_max_q_value = (float)1 / (1 - gamma) *
                  std::accumulate(knob::le_featurewise_num_tilings.begin(),
                                  knob::le_featurewise_num_tilings.end(), 0);
                                                 ...
                                                 vector<bool> &consensus_vec)
{
  ...
  if (m_type == LearningType::SARSA && m_policy == Policy::EGreedy) {
    if ((*m_explore)(m_generator)) {
      ...
      MYLOG("action taken %u explore, state %s, scores %s", action,
            state->to_string().c_str(), getStringQ(state).c_str());
    } else {
      float max_q = 0.0;
      ...
      gather_stats(max_q,
                   max_to_avg_q_ratio); /* for only stats collection's sake */
      MYLOG("action taken %u exploit, state %s, scores %s", action,
            state->to_string().c_str(), getStringQ(state).c_str());
    }
  } else {
...
void LearningEngineFeaturewise::learn(State *state1, uint32_t action1,
                                      int32_t reward, State *state2,
                                      uint32_t     action2,
                                      vector<bool> consensus_vec,
    ...
    for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
      if (m_feature_knowledges[index]) {
        if (!knob::le_featurewise_selective_update || consensus_vec[index]) {
          m_feature_knowledges[index]->updateQ(state1, action1, reward, state2,
                                               action2);
        } else if (knob::le_featurewise_selective_update &&
                   !consensus_vec[index]) {
                                                 ...
                                                 vector<bool> &consensus_vec)
{
  ...
  if ((max_q_value > 0 && avg_q_value > 0) ||
      (max_q_value < 0 && avg_q_value < 0)) {
...
void LearningEngineFeaturewise::dump_stats()
{
  ...
  fprintf(stdout, "learning_engine_featurewise.action.fallback %lu\n",
          stats.action.fallback);
  fprintf(stdout,
          "learning_engine_featurewise.action.dyn_fallback_saved_bw %lu\n",
          stats.action.dyn_fallback_saved_bw);
  fprintf(stdout,
          "learning_engine_featurewise.action.dyn_fallback_saved_bw_acc %lu\n",
          stats.action.dyn_fallback_saved_bw_acc);
  for (uint32_t action = 0; action < m_actions; ++action) {
    fprintf(stdout,
            "learning_engine_featurewise.action.index_%d_explored %lu\n",
            scooby->getAction(action), stats.action.dist[action][0]);
    fprintf(stdout,
            "learning_engine_featurewise.action.index_%d_exploited %lu\n",
  ...
  for (uint32_t index = 0; index < m_q_value_histogram.size(); ++index) {
    fprintf(stdout,
            "learning_engine_featurewise.q_value_histogram.bucket_%u %lu\n",
  ...
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    if (m_feature_knowledges[index]) {
      fprintf(stdout,
              "learning_engine_featurewise.consensus.feature_align_%s %lu\n",
              FeatureKnowledge::getFeatureString((FeatureType)index).c_str(),
              stats.consensus.feature_align_dist[index]);
      fprintf(stdout,
              "learning_engine_featurewise.consensus.feature_align_%s_ratio "
              "%0.2f\n",
              FeatureKnowledge::getFeatureString((FeatureType)index).c_str(),
              (float)stats.consensus.feature_align_dist[index] /
                  stats.consensus.total);
    }
  }
  fprintf(stdout,
          "learning_engine_featurewise.consensus.feature_align_all %lu\n",
          stats.consensus.feature_align_all);
  fprintf(
      stdout,
      "learning_engine_featurewise.consensus.feature_align_all_ratio %0.2f\n",
  ...
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    if (m_feature_knowledges[index]) {
      fprintf(stdout,
              "learning_engine_featurewise.feature_%s_min_weight %0.8f\n",
              FeatureKnowledge::getFeatureString((FeatureType)index).c_str(),
              m_feature_knowledges[index]->get_min_weight());
      fprintf(stdout,
              "learning_engine_featurewise.feature_%s_max_weight %0.8f\n",
              FeatureKnowledge::getFeatureString((FeatureType)index).c_str(),
...
void LearningEngineFeaturewise::gather_stats(float max_q,
                                             float max_to_avg_q_ratio)
{
  ...
  for (uint32_t index = 0; index < m_q_value_buckets.size(); ++index) {
    ...
    high = (index < m_q_value_buckets.size() - 1) ? m_q_value_buckets[index + 1]
                                                  : +1000000000;
...
void LearningEngineFeaturewise::action_selection_consensus(
    State *state, uint32_t selected_action, vector<bool> &consensus_vec)
{
  stats.consensus.total++;
...
void LearningEngineFeaturewise::adjust_feature_weights(
    vector<bool> consensus_vec, RewardType reward_type)
{
  ...
  if (knob::le_featurewise_disable_adjust_weight_all_features_align &&
      (uint32_t)std::accumulate(consensus_vec.begin(), consensus_vec.end(),
                                0) ==
          knob::le_featurewise_active_features.size()) {
    return;
  }
...
bool LearningEngineFeaturewise::do_fallback(State *state)
{
  ...
  if (state->is_high_bw) {
    stats.action.dyn_fallback_saved_bw++;
    return false;
  } else if (state->bw_level >= knob::le_featurewise_bw_acc_check_level &&
             state->acc_level <= knob::le_featurewise_acc_thresh) {
    stats.action.dyn_fallback_saved_bw_acc++;
    return false;
  }
...
void LearningEngineFeaturewise::plot_scores()
{
  ...
  fprintf(
      script,
      "set term pdf size 5,4 enhanced color font 'SVBasicManual, 20' lw 2\n");
  ...
  fprintf(script, "plot ");
  for (uint32_t index = 0; index < knob::le_featurewise_plot_actions.size();
       ++index) {
    if (index) {
      fprintf(script, ", ");
    }
    ...
    fprintf(script, "'%s' using 1:%u with lines ls %u title \"(%s)\"",
            ...
            (knob::le_featurewise_plot_actions[index] + 2), (index + 1),
            ss.str().c_str());
  }
  fprintf(script, "\n");
  fclose(script);
