                               ...
                               State *state2, uint32_t action2)
{
  ...
  MYLOG("<feature %s> Q(%s,%u) = %0.2f, R = %d, Q(%s,%u) = %0.2f, Q(%s,%u) = "
        ...
        action1, QSa1_old_overall, reward, state2->to_string().c_str(), action2,
        QSa2_old_overall, state1->to_string().c_str(), action1,
        QSa1_new_overall);
...
uint32_t FeatureKnowledge::get_tile_index(uint32_t tiling, State *state)
{
  ...
  case F_PC_Path_Offset_Path_Delta_Path:
    return process_PC_Path_Offset_Path_Delta_Path(tiling, pc_path, offset_path,
                                                  delta_path);
