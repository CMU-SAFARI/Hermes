...
void DSPatch::init_knobs()
{
  ...
  assert(knob::dspatch_num_cachelines_in_region *
...
DSPatch::DSPatch(string type) : Prefetcher(type)
{
  ...
  /* init SPT */
  spt = (DSPatch_SPTEntry **)calloc(knob::dspatch_num_spt_entries,
                                    sizeof(DSPatch_SPTEntry **));
  assert(spt);
...
void DSPatch::print_config()
{
  cout << "dspatch_log2_region_size " << knob::dspatch_log2_region_size << endl
       ...
       << knob::dspatch_num_cachelines_in_region << endl
       ...
       << knob::dspatch_compression_granularity << endl
       << "dspatch_pred_throttle_bw_thr " << knob::dspatch_pred_throttle_bw_thr
       << endl
       << "dspatch_bitmap_selection_policy "
       << knob::dspatch_bitmap_selection_policy << endl
       ...
       << "dspatch_enable_pref_buffer " << knob::dspatch_enable_pref_buffer
       << endl
                                ...
                                vector<uint64_t> &pref_addr)
{
  ...
  uint32_t offset =
      ...
      ((1ull << (knob::dspatch_log2_region_size - LOG2_BLOCK_SIZE)) - 1);
                                ...
                                uint64_t address, vector<uint64_t> &pref_addr)
{
  ...
  MYLOG("pc %lx sig %lx spt_index %u candidate %s", pc, signature, spt_index,
        Map_DSPatch_pref_candidate(candidate));

  /* decompress and rotate back the bitmap */
  MYLOG("orig bitmap %s", BitmapHelper::to_string(
                              bmp_pred, knob::dspatch_num_cachelines_in_region)
                              .c_str());
  bmp_pred =
      BitmapHelper::decompress(bmp_pred, knob::dspatch_compression_granularity,
                               knob::dspatch_num_cachelines_in_region);
  bmp_pred = BitmapHelper::rotate_left(bmp_pred, offset,
                                       knob::dspatch_num_cachelines_in_region);
  MYLOG(
      "rotated bitmap %s",
      BitmapHelper::to_string(bmp_pred, knob::dspatch_num_cachelines_in_region)
          .c_str());

  /* Throttling predictions incase of predicting with bmp_acc and b/w is high */
  if (bw_bucket >= knob::dspatch_pred_throttle_bw_thr &&
  ...
  MYLOG("pred bitmap %s", BitmapHelper::to_string(
                              bmp_pred, knob::dspatch_num_cachelines_in_region)
                              .c_str());
  for (uint32_t index = 0; index < knob::dspatch_num_cachelines_in_region;
       ++index) {
    if (bmp_pred[index] && index != offset) {
      uint64_t addr =
          (page << knob::dspatch_log2_region_size) + (index << LOG2_BLOCK_SIZE);
                                              ...
                                              Bitmap           &bmp_selected)
{
  ...
  case 3:
    ...
    break;
  default:
    cout << "invalid dspatch_bitmap_selection_policy "
         << knob::dspatch_bitmap_selection_policy << endl;
    assert(false);
  }
  return candidate;
...
{
  auto it = find_if(
      page_buffer.begin(), page_buffer.end(),
...
void DSPatch::add_to_spt(DSPatch_PBEntry *pbentry)
{
  ...
  MYLOG("page %lx trigger_pc %lx trigger_offset %u sig %lx spt_index %u",
        pbentry->page, trigger_pc, trigger_offset, signature, spt_index);
  ...
  uint32_t same_count_bmp_cov =
      BitmapHelper::count_bits_same(bmp_cov, bmp_real);
  uint32_t same_count_bmp_acc =
      BitmapHelper::count_bits_same(bmp_acc, bmp_real);
  ...
  MYLOG("bmp_real %s", BitmapHelper::to_string(
                           bmp_real, knob::dspatch_num_cachelines_in_region)
                           .c_str());
  MYLOG("bmp_cov  %s",
        BitmapHelper::to_string(bmp_cov, knob::dspatch_num_cachelines_in_region)
            .c_str());
  MYLOG("bmp_acc  %s",
        BitmapHelper::to_string(bmp_acc, knob::dspatch_num_cachelines_in_region)
            .c_str());
  MYLOG("cov_bmp_cov %u acc_bmp_acc %u cov_bmp_acc %u acc_bmp_acc %u",
        cov_bmp_cov, acc_bmp_cov, cov_bmp_acc, acc_bmp_acc);
  ...
  if (acc_bmp_cov < knob::dspatch_acc_thr ||
      cov_bmp_cov < knob::dspatch_cov_thr) {
    sptentry->measure_covP.incr(knob::dspatch_measure_covP_max);
    stats.spt.measure_covP_incr++;
  }

  /* Update CovP */
  MYLOG("sptentry->bmp_cov before %s",
        BitmapHelper::to_string(sptentry->bmp_cov,
                                knob::dspatch_num_cachelines_in_region)
            .c_str());
  if (sptentry->measure_covP.value() == knob::dspatch_measure_covP_max) {
    ...
    if (bw_bucket == 3 || cov_bmp_cov < 50) /* WARNING: hardcoded values */
    {
      ...
      sptentry->bmp_cov = BitmapHelper::compress(
          bmp_real, knob::dspatch_compression_granularity);
  ...
  MYLOG("sptentry->bmp_cov after  %s",
        BitmapHelper::to_string(sptentry->bmp_cov,
                                knob::dspatch_num_cachelines_in_region)
            .c_str());
  ...
  /* Update AccP */
  MYLOG("sptentry->bmp_acc before %s",
        BitmapHelper::to_string(sptentry->bmp_acc,
                                knob::dspatch_num_cachelines_in_region)
            .c_str());
  sptentry->bmp_acc = BitmapHelper::bitwise_and(
      bmp_real, BitmapHelper::decompress(
                    sptentry->bmp_cov, knob::dspatch_compression_granularity,
                    knob::dspatch_num_cachelines_in_region));
  sptentry->bmp_acc = BitmapHelper::compress(
      sptentry->bmp_acc, knob::dspatch_compression_granularity);
  MYLOG("sptentry->bmp_acc after  %s",
        BitmapHelper::to_string(sptentry->bmp_acc,
                                knob::dspatch_num_cachelines_in_region)
            .c_str());
...
void DSPatch::dump_stats()
{
  cout << "dspatch.pb.lookup " << stats.pb.lookup << endl
       ...
       << "dspatch.dyn_selection.accp_reason1 "
       << stats.dyn_selection.accp_reason1 << endl
       << "dspatch.dyn_selection.accp_reason2 "
       << stats.dyn_selection.accp_reason2 << endl
       << "dspatch.dyn_selection.covp_reason1 "
       << stats.dyn_selection.covp_reason1 << endl
       << "dspatch.dyn_selection.covp_reason2 "
       << stats.dyn_selection.covp_reason2 << endl
       << endl
       << "dspatch.spt.called " << stats.spt.called << endl
       << "dspatch.spt.or_count_incr " << stats.spt.or_count_incr << endl
       << "dspatch.spt.measure_covP_incr " << stats.spt.measure_covP_incr
       << endl
       << "dspatch.spt.bmp_cov_reset " << stats.spt.bmp_cov_reset << endl
       << "dspatch.spt.bmp_cov_update " << stats.spt.bmp_cov_update << endl
       << "dspatch.spt.measure_accP_incr " << stats.spt.measure_accP_incr
       << endl
       << "dspatch.spt.measure_accP_decr " << stats.spt.measure_accP_decr
       << endl
