...
void PATTERN_TABLE::read_pattern(uint32_t curr_sig, int *delta_q,
                                 ...
                                 uint32_t &depth, GLOBAL_REGISTER &GHR)
{
  // Update (sig, delta) correlation
  ...
  if (c_sig[set]) {
    for (uint32_t way = 0; way < PT_WAY; way++) {
      ...
      if (pf_conf >= knob::spp_dev2_pf_threshold) {
        ...
        SPP_DP(cout << "[PT] " << __func__
                    << " PF_THRESH: " << knob::spp_dev2_pf_threshold
                    << " HIGH CONF: " << pf_conf << " sig: " << hex << curr_sig
                    << dec << " set: " << set << " way: " << way;
        ...
        SPP_DP(cout << "[PT] " << __func__
                    << " PF_THRESH: " << knob::spp_dev2_pf_threshold
                    << "  LOW CONF: " << pf_conf << " sig: " << hex << curr_sig
                    << dec << " set: " << set << " way: " << way;
               cout << " delta: " << delta[set][way] << " c_delta: "
                    << c_delta[set][way] << " c_sig: " << c_sig[set];
               cout << " conf: " << local_conf << " depth: " << depth << endl;);
