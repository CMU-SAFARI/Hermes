...
void NextLinePrefetcher::print_config()
{
  ...
  cout << "next_line_seed " << knob::next_line_seed << endl
       ...
       << "next_line_enable_prefetch_tracking "
       << knob::next_line_enable_prefetch_tracking << endl
...
void NextLinePrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address,
                                           ...
                                           vector<uint64_t> &pref_addr)
{
  ...
  for (uint32_t deg = 1; deg <= knob::next_line_pref_degree; ++deg) {
    ...
    if (prefetched_offset >= 0 && prefetched_offset < 64) {
      uint64_t addr =
          (page << LOG2_PAGE_SIZE) + (prefetched_offset << LOG2_BLOCK_SIZE);
      if (knob::next_line_enable_prefetch_tracking) {
...
{
  auto it = find_if(
      prefetch_tracker.begin(), prefetch_tracker.end(),
      [address](NL_PTEntry *ptentry) { return ptentry->address == address; });
...
void NextLinePrefetcher::measure_stats(NL_PTEntry *ptentry)
{
  ...
  if (knob::next_line_enable_trace &&
      trace_interval++ == knob::next_line_trace_interval) {
    trace_timestamp++;
...
{
  for (uint32_t index = 0; index < knob::next_line_deltas.size(); ++index) {
    cout << "next_line_delta_" << knob::next_line_deltas[index] << "_select "
         << stats.predict.select[index] << endl;
    cout << "next_line_delta_" << knob::next_line_deltas[index] << "_issue "
         << stats.predict.issue[index] << endl;
    cout << "next_line_delta_" << knob::next_line_deltas[index]
         << "_tracker_hit " << stats.predict.tracker_hit[index] << endl;
    cout << "next_line_delta_" << knob::next_line_deltas[index]
         << "_out_of_bounds " << stats.predict.out_of_bounds[index] << endl;
  }
  ...
  cout << "next_line_track_called " << stats.track.called << endl
       ...
       << "next_line_register_fill_called " << stats.register_fill.called
       << endl
       << "next_line_register_fill_fill " << stats.register_fill.fill << endl
       << "next_line_record_demand_called " << stats.record_demand.called
       << endl
       << "next_line_record_demand_hit " << stats.record_demand.hit << endl
       << "next_line_record_demand_timely " << stats.record_demand.timely
       << endl
       << "next_line_record_demand_untimely " << stats.record_demand.untimely
       << endl
