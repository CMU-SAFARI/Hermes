...
void SMSPrefetcher::invoke_prefetcher(uint64_t pc, uint64_t address,
                                      ...
                                      vector<uint64_t> &pref_addr)
{
  ...
  uint32_t offset =
      ...
      ((1ull << (knob::sms_region_size_log - LOG2_BLOCK_SIZE)) - 1);
...
{
  return find_if(filter_table.begin(), filter_table.end(),
                 [page](FTEntry *ftentry) { return (ftentry->page == page); });
}

void SMSPrefetcher::insert_filter_table(uint64_t pc, uint64_t page,
                                        uint32_t offset)
{
  stats.ft.insert++;
...
{
  return find_if(acc_table.begin(), acc_table.end(),
                 [page](ATEntry *atentry) { return (atentry->page == page); });
}
                                                      ...
                                                      int32_t *set)
{
  ...
  return find_if(pht[(*set)].begin(), pht[(*set)].end(),
                 [signature](PHTEntry *phtentry) {
                   return (phtentry->signature == signature);
                 });
}
...
void SMSPrefetcher::update_age_pht(int32_t                     set,
                                   deque<PHTEntry *>::iterator current)
{
...
int SMSPrefetcher::generate_prefetch(uint64_t pc, uint64_t address,
                                     ...
                                     vector<uint64_t> &pref_addr)
{
  ...
  for (uint32_t index = 0; index < BITMAP_MAX_SIZE; ++index) {
    if (phtentry->pattern[index] && offset != index) {
      uint64_t addr =
          (page << knob::sms_region_size_log) + (index << LOG2_BLOCK_SIZE);
...
void SMSPrefetcher::dump_stats()
{
  cout << "sms.ft.lookup " << stats.ft.lookup << endl
       ...
       << "sms.generate_prefetch.called " << stats.generate_prefetch.called
       << endl
       << "sms.generate_prefetch.pht_miss " << stats.generate_prefetch.pht_miss
       << endl
       << "sms.generate_prefetch.pref_generated "
       << stats.generate_prefetch.pref_generated << endl
