...
uint32_t LearningEngineCMAC2::process_delta(int32_t delta, uint32_t plane)
{
  ...
  uint32_t unsigned_delta =
      (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1)))
                  : delta; /* converts into 7 bit signed representation */
...
uint32_t LearningEngineCMAC2::process_offset_path(uint32_t offset_path,
                                                  uint32_t plane)
{
...
uint32_t LearningEngineCMAC2::process_delta_path(uint32_t delta_path,
                                                 uint32_t plane)
{
...
uint32_t LearningEngineCMAC2::get_processed_feature(Feature  feature,
                                                    State   *state,
                                                    uint32_t plane)
{
...
uint32_t LearningEngineCMAC2::gen_state_generic(uint32_t plane, State *state)
{
  ...
  for (uint32_t index = 0; index < knob::le_cmac2_active_features.size();
       ++index) {
    initial_index <<= knob::le_cmac2_feature_shift_amount;
    initial_index += get_processed_feature(
        (Feature)knob::le_cmac2_active_features[index], state, plane);
  }
