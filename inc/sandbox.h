...
private:
  ...
  void     reset_eval();
  void     get_offset_list_sorted(vector<Score *> &pos_offsets,
                                  vector<Score *> &neg_offsets);
  void     generate_prefetch(vector<Score *> offset_list, uint32_t pref_degree,
                             uint64_t page, uint32_t offset,
                             vector<uint64_t> &pref_addr);
  void     destroy_offset_list(vector<Score *> offset_list);
  uint64_t generate_address(uint64_t page, uint32_t offset, int32_t delta,
