...
{
public:
  int dan_promotion_threshold = 256, dan_init_weight = 1, dan_dt1 = 55,
      dan_dt2 = 1024, dan_rrip_place_position = 0, dan_leaders = 34,
...
struct perceptron_predictor {
  ...
  void block_is_dead(uint32_t tid, sdbp_sampler_entry *, unsigned int *, bool,
                     int, int);
...
struct sdbp_sampler {
  ...
  unsigned int   trace_buffer[MAX_PATH_LENGTH +
                            1];  // trace is built here before prediction
  unsigned int **addresses;        // per-core array of recent PCs
  int            lognsets6;

  sdbp_sampler(mp3b_params *params, int nsets, int assoc);
  void access(uint32_t tid, int set, int real_set, uint64_t tag, uint64_t PC,
              int, uint64_t);
  void make_trace(uint32_t tid, perceptron_predictor *pred, uint32_t setIndex,
                  uint64_t PC, uint32_t tag, int accessType, bool burst,
