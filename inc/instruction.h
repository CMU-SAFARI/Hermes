...
public:
  ...
  input_instr_v2() { memset(this, 0, sizeof(*this)); };
};
...
public:
  ...
  uint8_t is_branch, is_memory, is_load, is_store, branch_taken,
      branch_mispredicted, branch_prediction_made, translated, data_translated,
  ...
  uint8_t mem_source_went_offchip
      [NUM_INSTR_SOURCES];  // RBERA: to track which memory sources went offchip
  ...
  string to_string()
  {
    ...
    ss << "|" << hex << std::setw(8) << instr_id << dec << "|" << hex
       << std::setw(8) << (uint64_t)ip << dec << "|" << "<"
       << (uint32_t)is_branch << "-" << (uint32_t)branch_mispredicted << "-"
       << (uint32_t)branch_taken << ">" << "<" << (uint32_t)is_producer << "-"
       << (uint32_t)is_consumer << "-" << (uint32_t)reg_RAW_producer << ">"
       << "<" << (uint64_t)is_memory << "-" << (uint32_t)is_load << "-"
       << (uint32_t)is_store << ">" << "|";
...
static_assert(sizeof(input_instr_v2) == 512,
              "input_instr_v2 must be 512 bytes");
