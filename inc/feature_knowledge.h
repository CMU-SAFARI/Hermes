...
private:
  ...
  uint32_t process_PC_offset_delta(uint32_t tiling, uint64_t pc,
                                   uint32_t offset, int32_t delta);
  uint32_t process_Page(uint32_t tiling, uint64_t page);
  uint32_t process_PC_Path_Offset(uint32_t tiling, uint32_t pc_path,
                                  uint32_t offset);
  uint32_t process_PC_Path_Offset_Path(uint32_t tiling, uint32_t pc_path,
                                       uint32_t offset_path);
  uint32_t process_PC_Path_Delta(uint32_t tiling, uint32_t pc_path,
                                 int32_t delta);
  uint32_t process_PC_Path_Delta_Path(uint32_t tiling, uint32_t pc_path,
                                      uint32_t delta_path);
  uint32_t process_PC_Path_Offset_Path_Delta_Path(uint32_t tiling,
                                                  uint32_t pc_path,
                                                  uint32_t offset_path,
                                                  uint32_t delta_path);
  uint32_t process_Offset_Path_PC(uint32_t tiling, uint32_t offset_path,
                                  uint64_t pc);
  uint32_t process_Delta_Path_PC(uint32_t tiling, uint32_t delta_path,
                                 uint64_t pc);
...
public:
  FeatureKnowledge(FeatureType feature_type, float alpha, float gamma,
                   uint32_t actions, float weight, float weight_gradient,
                   uint32_t num_tilings, uint32_t num_tiles, bool zero_init,
                   uint32_t hash_type, int32_t enable_tiling_offset);
  ~FeatureKnowledge();
  float retrieveQ(State *state, uint32_t action_index);
  void  updateQ(State *state1, uint32_t action1, int32_t reward, State *state2,
                uint32_t action2);
  static string getFeatureString(FeatureType type);
  uint32_t
  getMaxAction(State *state); /* Called by featurewise engine only to get a
                                 consensus from all the features */
