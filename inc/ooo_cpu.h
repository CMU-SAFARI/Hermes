...
public:
  ...
  unordered_map<uint64_t, load_per_ip_info_t> load_per_ip_stats,
  ...
  void reset_stats()
  {
    ...
    if (offchip_pred) {
      offchip_pred
          ->reset_stats();  // NULL in uncore mode (LLC owns the predictor)
    }
    if (ddrp_monitor) {
  ...
  // RBERA: new fuctions
  uint32_t       count_dependency(uint32_t               rob_index,
                                  std::vector<uint32_t> &cat_dependents);
  void           count_dependency_recursive(uint32_t               rob_index,
  ...
  {
    return (rob_head <= rob_index) ? (rob_index - rob_head)
                                   : (ROB_SIZE - rob_head + rob_index);
  }
  ...
  void initialize_offchip_predictor(uint64_t seed),
      dump_stats_offchip_predictor(), print_config_offchip_predictor(),
      offchip_predictor_update_dram_bw(uint8_t dram_bw),
      offchip_predictor_track_llc_eviction(uint32_t set, uint32_t way,
                                           uint64_t address);
