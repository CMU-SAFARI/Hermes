...
class OffchipPredBase;    // uncore-owned off-chip predictor (defined via
                          // offchip_pred_base.h)
class MEMORY_CONTROLLER;  // direct line to DRAM for DDRP (defined in
                          // dram_controller.h)
...
public:
  ...
  // direct line from this cache straight to the DRAM controller (for DDRP,
  // which bypasses the cache hierarchy). Linked in main.cc; NULL otherwise.
  ...
  void add_mshr(PACKET *packet), update_fill_cycle(),
      llc_initialize_replacement(), llc_replacement_print_config(),
  ...
  void l1d_prefetcher_print_config(), l2c_prefetcher_print_config(),
      llc_prefetcher_print_config();

  uint32_t l1d_prefetcher_prefetch_hit(uint64_t addr, uint64_t ip,
                                       uint32_t metadata_in),
      l2c_prefetcher_prefetch_hit(uint64_t addr, uint64_t ip,
                                  uint32_t metadata_in),
      llc_prefetcher_prefetch_hit(uint64_t addr, uint64_t ip,
                                  uint32_t metadata_in);
  ...
  void prefetcher_feedback(uint64_t &pref_gen, uint64_t &pref_fill,
