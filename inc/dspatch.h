...
private:
  ...
  struct {
    ...
    struct {
      uint64_t called;
      uint64_t
          selection_dist[DSPatch_pref_candidate::Num_DSPatch_pref_candidates];
      uint64_t reset;
      uint64_t total;
    } gen_pref;
...
private:
  void                   init_knobs();
  void                   init_stats();
  DSPatch_pref_candidate select_bitmap(DSPatch_SPTEntry *sptentry,
                                       Bitmap           &bmp_selected);
  ...
  DSPatch_pref_candidate dyn_selection(DSPatch_SPTEntry *sptentry,
                                       Bitmap           &bmp_selected);
  void generate_prefetch(uint64_t pc, uint64_t page, uint32_t offset,
