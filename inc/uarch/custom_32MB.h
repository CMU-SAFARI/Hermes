/******************************************************************
 * Custom capacity-sweep configuration. Core parameters are the Golden Cove
 * model from glc.h, unchanged; only the cache hierarchy is re-cut:
 *
 *   L2   1 MB = 1024 sets x 16 ways x 64 B
 *   LLC 32 MB = 32768 sets x 16 ways x 64 B
 *
 * 16-way at both levels makes every LLC sweep point (1/2/4/8/16/32 MB) a
 * power-of-two set count, which CACHE::get_set requires: it masks with
 * (1 << lg2(NUM_SET)) - 1, so a non-power-of-two count silently strands
 * capacity. 2 MB is the baseline.
 *
...
// L2 CACHE
#define L2C_SET       1024  // 1 MB per-core (1024 x 16 x 64B)
#define L2C_WAY       16
...
// LAST LEVEL CACHE
#define LLC_SET       NUM_CPUS * 32768  // 32MB per core (32768 x 16 x 64B)
#define LLC_WAY       16
