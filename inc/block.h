...
public:
  ...
  uint32_t cpu, occupancy, num_returned, next_fill_index, next_schedule_index,
      next_process_index;
  ...
  void reset_stats() { bzero(&stats, sizeof(stats)); }
...
public:
  ...
  // destructor
  virtual ~PACKET_QUEUE() { delete[] entry; };
...
public:
  ...
  int32_t head;
  deque<PACKET>::iterator
                  head_iterator;  // iterator to head; used in remove_queue()
  priority_type_t priority_type;

  deque<PACKET> entry;

  // constructors
  PACKET_QUEUE_PRIORITY(string v1, uint32_t v2, priority_type_t v3)
      : PACKET_QUEUE_BASE(v1, v2), priority_type(v3)
  {
    entry.clear();
    occupancy = 0;
    head      = -1;
  ...
  PACKET_QUEUE_PRIORITY(priority_type_t v1)
      : PACKET_QUEUE_BASE(), priority_type(v1)
  {
    head      = -1;
    occupancy = 0;
  }
...
private:
  ...
  int32_t compare_priorty(deque<PACKET>::iterator it1,
