...
| 2026-07-13 | charts/speedup_sizes_membound146_bw3200.{png,pdf} | scripts/plot_speedup_sizes_membound146.R | sizes_ipc_bw3200 (raw) -> sizes_speedup_bw3200 | Hermes-UnC size comparison Lite/Normal/Big +-Pythia + Pythia, 146 mem-intensive, bw3200 full window. Lite-alone/Big-alone from sizes_alone batch; +Pythia sizes reuse E3' (binary-identical). GEOMEAN: sizes flat (all alone 1.058; +Pythia 1.169/1.171/1.171) — differentiate on prediction quality/multi-core, not single-core speed |
| 2026-07-13 | charts/speedup_ext_membound146_bw3200.{png,pdf} | scripts/plot_speedup_membound146_ext.R | extra_ipc_bw3200 (raw) -> numbers csv | EXTENDED 9-bar speedup: base 5 + NoPC + Core-P (+-Pythia). PRELIM n=135 (11 llvm_s node-loss retrying -> refresh to 146). GEOMEAN: NoPC~Hermes-UnC (1.052 vs 1.057); Core beats uncore standalone (1.066 vs 1.057) & +Pythia (1.176 vs 1.173) — quantifies placement cost of losing PC |
| 2026-07-13 | charts/accuracy_coverage_ext_{standalone,pythia}_bw3200.{png,pdf} | scripts/plot_accuracy_coverage_ext.R | acc_cov_ext_raw_bw3200 -> acc_cov_ext_{std,pyt}_bw3200 | Accuracy/coverage for 4 design points: XPT, Hermes-UnC, NoPC, Core (std + Pythia). Core prec/recall from Core_0_ keys (.out extract, not LLC mfile). NoPC/Core n=135-136 (11 llvm node-loss). Std AVG: NoPC highest coverage 96 but 91 acc; Core 91/92. +Pythia: Hermes-UnC best 85/70, NoPC degrades most 67/65, Core 79/69 |
| 2026-07-13 | charts/gap_pcless_vs_unc_pythia.{png,pdf} | scripts/plot_gap_pcless_vs_unc_pythia.R | gap_pcless_vs_unc_pyt | Sorted per-trace gap: (Hermes-UnC+Pythia - NoPC+Pythia) over Pythia, 135 traces. median 0.24/mean 0.61pp; range [-3.75,+6.25]; UnC wins 81, tie 24, NoPC 30; |gap|>2pp on 18. Near-equal average hides real per-trace spread; 3rd feature earns its keep on heavy mem workloads (pot3d/cactus/fotonik3d) |

## Terminology (2026-07-13): "Hermes-PCless9" renamed "Hermes-NoPC" everywhere.
- Hermes-Core = original proposal (core placement, PC-augmented features).
