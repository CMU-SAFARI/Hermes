...
sp <- stats %>% filter(ExpName == "proj_normal_pythia") %>% inner_join(base, by="TraceName") %>%
  mutate(speedup = ipc/base) %>% arrange(speedup) %>% mutate(rank = row_number())
write.csv(sp %>% select(rank, TraceName, speedup),
          file.path(here,"charts","scurve_speedint_hermes_over_pythia_numbers.csv"), row.names=FALSE)
...
col <- hermes_fill["Hermes-UnC+Pythia"]
p <- ggplot(sp, aes(rank, speedup)) +
  ...
  coord_cartesian(ylim=c(0.96, NA)) +
  labs(title="Hermes-UnC on top of Pythia - per-trace speedup S-curve (SPEC Speed-Int)",
       subtitle=sub, x="Speed-Int traces (sorted by speedup)", y="Speedup over Pythia") +
...
ggsave(file.path(here,"charts","scurve_speedint_hermes_over_pythia.png"), p, width=13, height=7, dpi=200, device=ragg::agg_png)
ggsave(file.path(here,"charts","scurve_speedint_hermes_over_pythia.pdf"), p, width=13, height=7, device=cairo_pdf)
message("wrote scurve_speedint_hermes_over_pythia")
