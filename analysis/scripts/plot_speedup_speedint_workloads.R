...
d <- yaml::read_yaml("/home/rahbera/thesis/runs/cluster/spec26.speed-int.yml")
...
write.csv(agg %>% select(workload, short, ExpName, n, geomean),
          file.path(here,"charts","speedup_speedint_workloads_numbers.csv"), row.names=FALSE)
...
p <- ggplot(agg, aes(short, geomean, fill = ExpName)) +
  ...
  labs(title = "SPEC Speed-Int per-workload speedup over nopref (full suite)",
...
ggsave(file.path(here,"charts","speedup_speedint_workloads.png"), p,
       width = 18, height = 8, dpi = 200, device = ragg::agg_png)
ggsave(file.path(here,"charts","speedup_speedint_workloads.pdf"), p,
       width = 18, height = 8, device = cairo_pdf)
message("Wrote speedup_speedint_workloads.{png,pdf}")
