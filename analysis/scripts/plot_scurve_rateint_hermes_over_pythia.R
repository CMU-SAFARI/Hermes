#!/usr/bin/env Rscript
# S-curve: what Hermes-UnC adds ON TOP OF Pythia. Per-trace speedup of
# Hermes-UnC+Pythia over Pythia, all 144 SPEC Rate-Int traces sorted low->high.
...
base <- stats %>% filter(ExpName == "proj_pythia") %>% select(TraceName, base = ipc)
sp <- stats %>% filter(ExpName == "proj_normal_pythia") %>%
...
write.csv(sp %>% select(rank, TraceName, speedup),
          file.path(here,"charts","scurve_rateint_hermes_over_pythia_numbers.csv"), row.names = FALSE)
...
col <- hermes_fill["Hermes-UnC+Pythia"]   # strong dark blue
p <- ggplot(sp, aes(rank, speedup)) +
  ...
  labs(title = "Hermes-UnC on top of Pythia — per-trace speedup S-curve (SPEC Rate-Int)",
       subtitle = sub,
       x = "Rate-Int traces (sorted by speedup)", y = "Speedup over Pythia") +
...
ggsave(file.path(here,"charts","scurve_rateint_hermes_over_pythia.png"), p,
       width = 13, height = 7, dpi = 200, device = ragg::agg_png)
ggsave(file.path(here,"charts","scurve_rateint_hermes_over_pythia.pdf"), p,
       width = 13, height = 7, device = cairo_pdf)
message("Wrote scurve_rateint_hermes_over_pythia.{png,pdf}")
