...
# (Hermes-PCless9+Pythia over Pythia), in pp, all mem-intensive traces sorted.
# Positive = the full 3-feature uncore predictor wins; negative = 1-feature
# PCless9 wins. Shows the spread hidden by the near-equal averages.
...
d <- d %>% arrange(gap_pp) %>% mutate(rank = row_number(),
        winner = ifelse(gap_pp >= 0, "Hermes-UnC+Pythia wins", "Hermes-PCless9+Pythia wins"))
pal <- c("Hermes-UnC+Pythia wins" = hermes_fill[["Hermes-UnC+Pythia"]],
         "Hermes-PCless9+Pythia wins" = hermes_fill[["Hermes-PCless9+Pythia"]])
sub <- sprintf(paste0("%d traces, gap = 3-feat uncore minus 1-feat PCless9 (speedup over Pythia).  ",
   "median %.2f, mean %.2f pp; range [%.2f, %.2f]; wins: UnC %d, tie %d, PCless9 %d."),
...
p <- ggplot(d, aes(rank, gap_pp, fill = winner)) +
  ...
  labs(title = "Hermes-UnC vs PCless9 (both + Pythia) — per-trace performance gap",
       subtitle = sub,
