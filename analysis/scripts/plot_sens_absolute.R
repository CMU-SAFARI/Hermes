...
tagmap<-bind_rows(tr)
lab <- c(bw3200_pythia="Pythia", sens_stride_pythia="Stride+Pythia",
         sens_stride_pythia_xpt="Stride+Pythia+XPT",
         sens_stride_pythia_unc="Stride+Pythia+Hermes-UnC",
         sens_stride_pythia_core="Stride+Pythia+Hermes-Core")
lev <- c("Pythia","Stride+Pythia","Stride+Pythia+XPT","Stride+Pythia+Hermes-UnC","Stride+Pythia+Hermes-Core")
base <- stats %>% filter(ExpName=="bw3200_nopf") %>% select(TraceName, b=ipc)
...
write.csv(agg, file.path(here,"charts","sens_absolute_numbers.csv"), row.names=FALSE)
...
p<-ggplot(agg,aes(category,geomean,fill=ExpName))+
  ...
  geom_text(data=filter(agg,category=="GEOMEAN"),aes(label=sprintf("%.3f",geomean),group=ExpName),
            position=dodge,vjust=0.5,hjust=-0.15,size=3.4,colour="grey15",angle=90)+
  ...
  labs(title="Multi-prefetcher sensitivity: speedup over nopref (Stride@L1D + Pythia@L2)",
       subtitle="146 mem-intensive traces, full window, 1 core, DDR-3200. Bars: geomean of per-trace IPC speedup over no-prefetch.",
       x=NULL,y="Geomean speedup over no-prefetching")+
...
ggsave(file.path(here,"charts","sens_absolute_membound146.png"),p,width=16,height=8,dpi=200,device=ragg::agg_png)
ggsave(file.path(here,"charts","sens_absolute_membound146.pdf"),p,width=16,height=8,device=cairo_pdf)
message("wrote sens_absolute")
