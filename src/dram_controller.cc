...
string MemControllerScheduleTypeString[] = {
    "FR_FCFS",          "FRONTAL_FR_FCFS",  "FR_FRONTAL_FCFS",
    "ROB_PART_FR_FCFS", "FR_ROB_PART_FCFS", "CR_FCFS"};
...
void print_dram_config()
{
  cout << "dram_channel_width " << DRAM_CHANNEL_WIDTH << endl
       ...
       << "dram_rq_schedule_type "
       << MemControllerScheduleTypeString[knob::dram_rq_schedule_type] << endl
       << "dram_force_rq_row_buffer_miss "
       << knob::dram_force_rq_row_buffer_miss << endl
       << endl;
}
...
void MEMORY_CONTROLLER::schedule(PACKET_QUEUE *queue)
{
                    ...
                    // already scheduled or all banks are busy
  {
    ...
    if (knob::enable_pseudo_direct_dram_prefetch &&
        queue->entry[index].is_data) {
      // model only for loads, and also for prefetch requests if enabled
      if (queue->entry[index].type == LOAD ||
          (queue->entry[index].type == PREFETCH &&
           knob::enable_pseudo_direct_dram_prefetch_on_prefetch)) {
        uint8_t op_rob_part_type =
            ooo_cpu[op_cpu].rob_pos_get_part_type(op_rob_pos);
        if (knob::pseudo_direct_dram_prefetch_rob_part_type ==
                NUM_PARTITION_TYPES ||
    ...
    DP(if (warmup_complete[op_cpu]) {
      cout << "[" << queue->NAME << "] " << __func__
           << " instr_id: " << queue->entry[index].instr_id;
      cout << " row buffer: "
           << (row_buffer_hit
                   ? (int)bank_request[op_channel][op_rank][op_bank].open_row
                   : -1)
           << hex;
      cout << " address: " << queue->entry[index].address
           << " full_addr: " << queue->entry[index].full_addr << dec;
      ...
      cout << " current: " << current_core_cycle[op_cpu]
           << " event: " << queue->entry[index].event_cycle << endl;
    });
  }
}

int MEMORY_CONTROLLER::find_next_request(PACKET_QUEUE *queue,
                                         uint8_t      &row_buffer_hit)
{
  if (queue->is_WQ) {
...
void MEMORY_CONTROLLER::process(PACKET_QUEUE *queue)
{
  ...
  if (bank_request[op_channel][op_rank][op_bank].cycle_available <=
    ...
    if (dbus_cycle_available[op_channel] <= current_core_cycle[op_cpu]) {
        ...
        if (queue->entry[request_index].fill_level < FILL_DDRP) {
          ...
          DDRP_DP(if (warmup_complete[op_cpu]) {
            ...
            cout << " instr_id: " << queue->entry[request_index].instr_id
                 << " address: " << hex << queue->entry[request_index].address
                 << dec;
            cout << " full_addr: " << hex
                 << queue->entry[request_index].full_addr << dec
                 << " type: " << +queue->entry[request_index].type
          ...
          assert(queue->entry[request_index].type ==
                 PREFETCH);  // this has to be a DDRP prefetch
      ...
      dbus_congested[op_channel][bank_request[op_channel][op_rank][op_bank]
  ...
  if (all_warmup_complete < NUM_CPUS && return_data_to_core) {
    ...
    DDRP_DP(if (warmup_complete[packet->cpu]) {
      ...
      cout << " full_addr: " << packet->full_addr << dec
           ...
           << " return_data_before_warmup" << endl;
    });
  ...
  if (wq_index != -1) {
    ...
    DDRP_DP(if (warmup_complete[packet->cpu]) {
      cout << "[" << NAME << "_RQ] " << __func__
           << " instr_id: " << packet->instr_id << " address: " << hex
           << packet->address;
      cout << " full_addr: " << packet->full_addr << dec << " DRAM_WQ_"
           << channel << "_FORWARD";
    });
  ...
  for (uint32_t index = 0; index < DRAM_RQ_SIZE; index++) {
    if (RQ[channel].entry[index].address == 0) {
      ...
      DDRP_DP(if (warmup_complete[packet->cpu]) {
        ...
        cout << " full_addr: " << packet->full_addr << dec
             << " type: " << +packet->type
             << " fill_level: " << packet->fill_level << " ADDED_IN_DRAM_RQ_"
             << channel;
        cout << " occupancy: " << RQ[channel].occupancy
             << " current: " << current_core_cycle[packet->cpu]
             << " event: " << packet->event_cycle << endl;
...
void MEMORY_CONTROLLER::insert_ddrp_buffer(uint64_t addr)
{
  ...
  auto     it  = find_if(ddrp_buffer[set].begin(), ddrp_buffer[set].end(),
                         [addr](uint64_t m_addr) { return m_addr == addr; });
