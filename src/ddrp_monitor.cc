...
{
  cout << "ddrp_monitor_exploit_epoch " << knob::ddrp_monitor_exploit_epoch
       << endl
       ...
       << "ddrp_monitor_enable_hysterisis "
       << knob::ddrp_monitor_enable_hysterisis << endl
       << endl;
}
    ...
    if (instr_count >= knob::ddrp_monitor_explore_epoch) {
      ...
      if (config == ddrp_monitor_config_t::DDRP_OFF_DP_OFF) {
        MYLOG(warmup_complete[cpu],
              "explore -> exploit cycles taken in last explore config: %ld",
              cycles[config]);
        phase  = ddrp_monitor_phase_t::EXPLOIT;
        config = get_winner_config();
        reset_cycles();
        set_params();
        MYLOG(warmup_complete[cpu], "winner config: %s",
              ddrp_monitor_config_string[config].c_str());
        stats.monitor.explore_to_exploit++;
      }
      // otherwise transition to next config
      else {
        phase = ddrp_monitor_phase_t::EXPLORE;
        MYLOG(warmup_complete[cpu], "explore config transition:: old_cfg: %s",
              ddrp_monitor_config_string[config].c_str());
        config = (ddrp_monitor_config_t)((uint32_t)config + 1);
        MYLOG(warmup_complete[cpu], "new_cfg: %s",
              ddrp_monitor_config_string[config].c_str());
        set_params();
...
ddrp_monitor_config_t DDRPMonitor::get_winner_config()
{
  ...
  MYLOG(warmup_complete[cpu],
        ...
        cycles[DDRP_ON_DP_ON], cycles[DDRP_ON_DP_OFF], cycles[DDRP_OFF_DP_ON],
        cycles[DDRP_OFF_DP_OFF]);
...
void DDRPMonitor::dump_stats()
{
  cout << "monitor_called " << stats.monitor.called << endl
       << "monitor_exploit_to_explore " << stats.monitor.exploit_to_explore
       << endl
       << "monitor_in_exploit " << stats.monitor.in_exploit << endl
       << "monitor_explore_to_exploit " << stats.monitor.explore_to_exploit
       << endl
       << "monitor_explore_another_config "
       << stats.monitor.explore_another_config << endl
       << "monitor_in_explore " << stats.monitor.in_explore << endl
       << endl
       << "winner_config_called " << stats.winner_config.called << endl
       << "winner_config_DDRP_ON_DP_ON "
       << stats.winner_config.histogram[DDRP_ON_DP_ON] << endl
       << "winner_config_DDRP_ON_DP_OFF "
       << stats.winner_config.histogram[DDRP_ON_DP_OFF] << endl
       << "winner_config_DDRP_OFF_DP_ON "
       << stats.winner_config.histogram[DDRP_OFF_DP_ON] << endl
       << "winner_config_DDRP_OFF_DP_OFF "
       << stats.winner_config.histogram[DDRP_OFF_DP_OFF] << endl
       << endl;

  if (knob::ddrp_monitor_enable_hysterisis) {
    for (uint32_t cfg = 0; cfg < NumDDRPMonitorConfigs; ++cfg) {
      cout << "monitor_hysterisis_cfg_match_" << cfg << " "
           << stats.hysterisis.cfg_match[cfg] << endl;
      for (uint32_t cfg2 = 0; cfg2 < NumDDRPMonitorConfigs; ++cfg2) {
        cout << "monitor_hysterisis_cfg_mismatch_" << cfg << "_change_from_"
             << cfg2 << " " << stats.hysterisis.cfg_mismatch_change[cfg][cfg2]
             << endl
             << "monitor_hysterisis_cfg_mismatch_" << cfg << "_unchange_from_"
             << cfg2 << " "
             << stats.hysterisis.cfg_mismatch_unchanged[cfg][cfg2] << endl
             << endl;
      }
    }
  }
