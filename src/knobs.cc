                ...
                const char *value)
{
  ...
  if (MATCH("", "config")) {
    ...
    knob::l2c_prefetcher_force_prefetch_at_llc =
    ...
    knob::track_load_hit_dependency_in_cache =
        !strcmp(value, "true") ? true : false;
    ...
    knob::llc_pseudo_perfect_enable_frontal =
        !strcmp(value, "true") ? true : false;
    ...
    knob::l2c_pseudo_perfect_enable_frontal =
        !strcmp(value, "true") ? true : false;
    ...
    assert(len == ROB_SIZE);
    len = 0;
    for (uint32_t index = 0; index < knob::rob_partition_size.size() - 1;
         ++index) {
      len += knob::rob_partition_size[index];
    ...
    for (uint32_t index = 0; index < knob::rob_frontal_partition_ids.size();
         ++index) {
      assert(knob::rob_frontal_partition_ids[index] <
    ...
    for (uint32_t index = 0; index < knob::rob_dorsal_partition_ids.size();
         ++index) {
      assert(knob::rob_dorsal_partition_ids[index] <
             (int32_t)knob::num_rob_partitions);
    }
  } else if (MATCH("", "enable_pseudo_direct_dram_prefetch")) {
    knob::enable_pseudo_direct_dram_prefetch =
        !strcmp(value, "true") ? true : false;
  ...
  // {
  // 	knob::enable_dynamic_packet_priority = !strcmp(value, "true") ? true :
  // false;
  // }
  ...
  /* Scooby */
    ...
    knob::scooby_enable_direct_pref_issue =
        !strcmp(value, "true") ? true : false;
  ...
  /* Offchip Predictor Perceptron */
    ...
    knob::ocp_perc_enable_dynamic_act_thresh =
        !strcmp(value, "true") ? true : false;
  ...
  /* Offchip Predictor TTP */
    ...
    knob::ocp_ttp_enable_track_llc_eviction =
        !strcmp(value, "true") ? true : false;
  }
  ...
  // }
    ...
    knob::ddrp_monitor_enable_hysterisis =
        !strcmp(value, "true") ? true : false;
  }
