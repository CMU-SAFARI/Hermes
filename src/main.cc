...
void print_core_roi_stats(uint32_t cpu)
{
  ...
  cout << "Core_" << cpu << "_bubble_called "
       << ooo_cpu[cpu].stats.bubble.called << endl
       << "Core_" << cpu << "_bubble_rob_non_head "
       << ooo_cpu[cpu].stats.bubble.rob_non_head << endl
       << "Core_" << cpu << "_bubble_rob_head "
       << ooo_cpu[cpu].stats.bubble.rob_head << endl
       << "Core_" << cpu << "_bubble_went_offchip "
       << ooo_cpu[cpu].stats.bubble.went_offchip << endl
       << "Core_" << cpu << "_bubble_went_offchip_rob_head "
       << ooo_cpu[cpu].stats.bubble.went_offchip_rob_head << endl
       << "Core_" << cpu << "_bubble_went_offchip_rob_non_head "
       << ooo_cpu[cpu].stats.bubble.went_offchip_rob_non_head << endl
       << endl;

  for (uint32_t index = 0; index < knob::num_rob_partitions; ++index) {
    cout << "Core_" << cpu << "_bubble_rob_part_" << index << "_max "
         << ooo_cpu[cpu].bubble_max[index] << endl
         << "Core_" << cpu << "_bubble_rob_part_" << index << "_min "
         << ooo_cpu[cpu].bubble_min[index] << endl
         << "Core_" << cpu << "_bubble_rob_part_" << index << "_cnt "
         << ooo_cpu[cpu].bubble_cnt[index] << endl
         << "Core_" << cpu << "_bubble_rob_part_" << index << "_tot "
         << ooo_cpu[cpu].bubble_tot[index] << endl
         << "Core_" << cpu << "_bubble_rob_part_" << index << "_avg "
         << (float)ooo_cpu[cpu].bubble_tot[index] /
                ooo_cpu[cpu].bubble_cnt[index]
         << endl;
  }
  cout << endl;
  ...
  for (auto it = ooo_cpu[cpu].load_per_ip_stats.begin();
       it != ooo_cpu[cpu].load_per_ip_stats.end(); ++it) {
    pairs.push_back(*it);
  }
  std::sort(pairs.begin(), pairs.end(),
            [](const std::pair<uint64_t, load_per_ip_info_t> &p1,
               const std::pair<uint64_t, load_per_ip_info_t> &p2) {
              return p1.second.loads_went_offchip >
                     p2.second.loads_went_offchip;
            });
  ...
  cout << "Core_" << cpu << "_total_loads " << total_loads << endl
       << "Core_" << cpu << "_total_loads_went_offchip "
       << total_loads_went_offchip << endl
       << "Core_" << cpu << "_total_frontal_loads "
       << ooo_cpu[cpu].load_per_rob_part_stats[FRONTAL].total_loads << endl
       << "Core_" << cpu << "_total_frontal_loads_went_offchip "
       << ooo_cpu[cpu].load_per_rob_part_stats[FRONTAL].loads_went_offchip
       << endl
       << "Core_" << cpu << "_total_dorsal_loads "
       << ooo_cpu[cpu].load_per_rob_part_stats[DORSAL].total_loads << endl
       << "Core_" << cpu << "_total_dorsal_loads_went_offchip "
       << ooo_cpu[cpu].load_per_rob_part_stats[DORSAL].loads_went_offchip
       << endl
       << "Core_" << cpu << "_total_none_loads "
       << ooo_cpu[cpu].load_per_rob_part_stats[NONE].total_loads << endl
       << "Core_" << cpu << "_total_none_loads_went_offchip "
       << ooo_cpu[cpu].load_per_rob_part_stats[NONE].loads_went_offchip << endl
       << endl;
  ...
  for (auto it = pairs.begin(); it != pairs.end(); ++it) {
    cout << setw(10) << hex << it->first << dec << "|" << setw(10)
         << it->second.total_loads << "|" << setw(10)
         << it->second.loads_went_offchip << "|";
    for (uint32_t i = 0; i < it->second.loads_went_offchip_pos_hist.size();
         ++i) {
      cout << setw(7) << std::fixed << std::setprecision(2)
           << (float)it->second.loads_went_offchip_pos_hist[i] /
                  it->second.loads_went_offchip * 100
           << "|";
    }
    cout << endl;
    ...
    for (uint32_t index = 0; index < knob::num_rob_partitions; ++index) {
      if (ooo_cpu[cpu].rob_part_id_is_frontal(index)) {
        frontal_loads_went_offchip +=
            it->second.loads_went_offchip_pos_hist[index];
      }
      if (ooo_cpu[cpu].rob_part_id_is_dorsal(index)) {
        dorsal_loads_went_offchip +=
            it->second.loads_went_offchip_pos_hist[index];
      }
    }
    ...
    for (uint32_t i = 0; i < 5; ++i) {
      // check frontal bias
      if ((float)frontal_loads_went_offchip / it->second.loads_went_offchip >=
          bias_values[i]) {
        ...
        frontal_bias_load_ips_off_chip_loads[i] +=
            it->second.loads_went_offchip;
      }

      // check dorsal bias
      if ((float)dorsal_loads_went_offchip / it->second.loads_went_offchip >=
          bias_values[i]) {
  ...
  for (uint32_t i = 0; i < 5; ++i) {
    cout << "Core_" << cpu << "_load_ips_biased_" << bias_values[i] << " "
         << (frontal_bias_load_ips[i] + dorsal_bias_load_ips[i]) << endl
         << "Core_" << cpu << "_load_ips_frontal_biased_" << bias_values[i]
         << " " << frontal_bias_load_ips[i] << endl
         << "Core_" << cpu << "_load_ips_frontal_biased_" << bias_values[i]
         << "_total_loads " << frontal_bias_load_ips_total_loads[i] << endl
         << "Core_" << cpu << "_load_ips_frontal_biased_" << bias_values[i]
         << "_off_chip_loads " << frontal_bias_load_ips_off_chip_loads[i]
         << endl
         << "Core_" << cpu << "_load_ips_dorsal_biased_" << bias_values[i]
         << " " << dorsal_bias_load_ips[i] << endl
         << "Core_" << cpu << "_load_ips_dorsal_biased_" << bias_values[i]
         << "_total_loads " << dorsal_bias_load_ips_total_loads[i] << endl
         << "Core_" << cpu << "_load_ips_dorsal_biased_" << bias_values[i]
         << "_off_chip_loads " << dorsal_bias_load_ips_off_chip_loads[i]
         << endl;
  }
  cout << endl;
  ...
  for (auto it = ooo_cpu[cpu].frontal_load_per_ip_stats.begin();
       it != ooo_cpu[cpu].frontal_load_per_ip_stats.end(); ++it) {
    pairs2.push_back(*it);
  }
  ...
  accum                                                  = 0;
  uint64_t frontal_load_generating_ips                   = 0,
           frontal_load_generating_ips_offchip_biased[5] = {0};
  for (auto it = pairs2.begin(); it != pairs2.end(); ++it) {
    frontal_load_generating_ips++;
    for (uint32_t i = 0; i < 5; ++i) {
      if ((float)it->second.loads_went_offchip / it->second.total_loads >=
          bias_values[i]) {
  ...
  cout << "Core_" << cpu << "_total_frontal_load_generating_ips "
       << frontal_load_generating_ips << endl;
  for (uint32_t i = 0; i < 5; ++i) {
    cout << "Core_" << cpu << "_frontal_load_generating_ips_offchip_biased_"
         << bias_values[i] << " "
         << frontal_load_generating_ips_offchip_biased[i] << endl;
  }
  cout << endl;

  // OFFCHIP PREDICTOR STATS. Owner reports them: core mode -> per-core
  // Core_*_offchip_pred_*; uncore mode -> the LLC-owned predictor's
  // LLC_offchip_pred_* (printed once).
  if (!knob::offchip_pred_location.compare("core")) {
  ...
  cout << "Core_" << cpu << "_DDRP_total " << ooo_cpu[cpu].stats.ddrp.total
       << endl
       << "Core_" << cpu << "_DDRP_issued_after_direct_translation "
       << ooo_cpu[cpu].stats.ddrp.issued[0] << endl
       << "Core_" << cpu << "_DDRP_issued_after_merged_translation "
       << ooo_cpu[cpu].stats.ddrp.issued[1] << endl
       << "Core_" << cpu << "_DDRP_dram_RQ_full "
       << ooo_cpu[cpu].stats.ddrp.dram_rq_full << endl
       << "Core_" << cpu << "_DDRP_dram_MSHR_full "
       << ooo_cpu[cpu].stats.ddrp.dram_mshr_full << endl
       << endl;
...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
  ...
  cout << "Core_" << cpu << "_" << cache->NAME << "_total_access "
       ...
       << endl
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_total "
       << cache->stats.eviction.total << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_atleast_one_reuse "
       << cache->stats.eviction.atleast_one_reuse << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_atleast_one_load_reuse "
       << cache->stats.eviction.atleast_one_reuse_cat[LOAD] << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_atleast_one_RFO_reuse "
       << cache->stats.eviction.atleast_one_reuse_cat[RFO] << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_atleast_one_prefetch_reuse "
       << cache->stats.eviction.atleast_one_reuse_cat[PREFETCH] << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_atleast_one_writeback_reuse "
       << cache->stats.eviction.atleast_one_reuse_cat[WRITEBACK] << endl
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_all_reuse_total "
       << cache->stats.eviction.all_reuse_total << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_all_reuse_max "
       << cache->stats.eviction.all_reuse_max << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_all_reuse_min "
       << cache->stats.eviction.all_reuse_min << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_all_reuse_avg "
       << (float)cache->stats.eviction.all_reuse_total /
              cache->stats.eviction.atleast_one_reuse
       << endl;

#ifdef PRINT_AUX_STATS
  for (uint32_t type = LOAD; type < NUM_TYPES; ++type) {
    cout << "Core_" << cpu << "_" << cache->NAME << "_eviction_"
         << type_names[type] << "_reuse_total "
         << cache->stats.eviction.cat_reuse_total[type] << endl
         << "Core_" << cpu << "_" << cache->NAME << "_eviction_"
         << type_names[type] << "_reuse_max "
         << cache->stats.eviction.cat_reuse_max[type] << endl
         << "Core_" << cpu << "_" << cache->NAME << "_eviction_"
         << type_names[type] << "_reuse_min "
         << cache->stats.eviction.cat_reuse_min[type] << endl
         << "Core_" << cpu << "_" << cache->NAME << "_eviction_"
         << type_names[type] << "_reuse_avg "
         << (float)cache->stats.eviction.cat_reuse_total[type] /
                cache->stats.eviction.atleast_one_reuse_cat[type]
         << endl;
  }

  cout << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_reuse_only_frontal "
       << cache->stats.eviction.reuse_only_frontal << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_reuse_only_dorsal "
       << cache->stats.eviction.reuse_only_dorsal << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_reuse_only_none "
       << cache->stats.eviction.reuse_only_none << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_reuse_mixed "
       << cache->stats.eviction.reuse_mixed << endl
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_all_total "
       << cache->stats.eviction.dep_all_total << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_all_max "
       << cache->stats.eviction.dep_all_max << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_all_min "
       << cache->stats.eviction.dep_all_min << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_dep_branch_mispred_total "
       << cache->stats.eviction.dep_branch_mispred_total << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_dep_branch_mispred_max "
       << cache->stats.eviction.dep_branch_mispred_max << endl
       << "Core_" << cpu << "_" << cache->NAME
       << "_eviction_dep_branch_mispred_min "
       << cache->stats.eviction.dep_branch_mispred_min << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_branch_total "
       << cache->stats.eviction.dep_branch_total << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_branch_max "
       << cache->stats.eviction.dep_branch_max << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_branch_min "
       << cache->stats.eviction.dep_branch_min << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_load_total "
       << cache->stats.eviction.dep_load_total << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_load_max "
       << cache->stats.eviction.dep_load_max << endl
       << "Core_" << cpu << "_" << cache->NAME << "_eviction_dep_load_min "
       << cache->stats.eviction.dep_load_min << endl
       << endl;

  std::vector<std::pair<uint64_t, uint64_t>> pairs;
  for (auto it = cache->dependent_map.begin(); it != cache->dependent_map.end();
       ++it) {
    pairs.push_back(*it);
  }
  std::sort(
  ...
  for (uint32_t index = 0; index < pairs.size(); ++index) {
    ...
    if (index == 2 || index == 4 || index == 9) {
      cout << "Core_" << cpu << "_" << cache->NAME << "_" << (index + 1)
           << "_highest_dependent_contri " << (float)sum / total * 100 << endl;
    }
  }
  cout << endl;
  ...
  for (int index = pairs.size() - 1; index >= 0;
       --index)  // pairs array is already sorted, hence the reverse order
  {
  ...
  cout << "Core_" << cpu << "_" << cache->NAME << "_dep_0 "
       << (float)dep_bucket[0] / cache->stats.eviction.atleast_one_reuse * 100
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_dep_1_2 "
       << (float)dep_bucket[1] / cache->stats.eviction.atleast_one_reuse * 100
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_dep_3_5 "
       << (float)dep_bucket[2] / cache->stats.eviction.atleast_one_reuse * 100
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_dep_6_9 "
       << (float)dep_bucket[3] / cache->stats.eviction.atleast_one_reuse * 100
       << endl
       << "Core_" << cpu << "_" << cache->NAME << "_dep_10_above "
       << (float)dep_bucket[4] / cache->stats.eviction.atleast_one_reuse * 100
       << endl
       << endl;
  ...
  if (cache->cache_type == IS_LLC) {
    ...
    cout << "Core_" << cpu << "_" << cache->NAME
         << "_missing_load_rob_pos_hist_total " << total_load_miss << endl;
    for (uint32_t index = 0; index < ROB_SIZE; ++index) {
      cout << "Core_" << cpu << "_" << cache->NAME
           << "_missing_load_rob_pos_hist_" << index << " "
           << cache->missing_load_rob_pos_hist[index] << endl;
    }
    cout << endl;
    ...
    sum              = 0;
    for (uint32_t index = 0; index < ROB_SIZE; ++index) {
      if (!knob::rob_partition_boundaries.empty() &&
          (int32_t)index == knob::rob_partition_boundaries[part_id]) {
        cout << "Core_" << cpu << "_" << cache->NAME
             << "_missing_load_rob_pos_hist_part_" << part_id << " " << sum
             << endl;
        cout << "Core_" << cpu << "_" << cache->NAME
...
void print_branch_stats()
{
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    cout << "Core_" << i << "_branch_pred_accuracy "
         << (100.0 *
             (ooo_cpu[i].num_branch - ooo_cpu[i].branch_mispredictions)) /
                ooo_cpu[i].num_branch
         << endl
         << "Core_" << i << "_branch_pred_mpki "
         << (1000.0 * ooo_cpu[i].branch_mispredictions) /
                (ooo_cpu[i].num_retired - ooo_cpu[i].warmup_instructions)
         << endl
         << "Core_" << i << "_avg_ROB_occupancy_at_mispred "
         << (1.0 * ooo_cpu[i].total_rob_occupancy_at_branch_mispredict) /
                ooo_cpu[i].branch_mispredictions
         << endl
         << endl
         << "Core_" << i << "_not_branch " << ooo_cpu[i].total_branch_types[0]
         << endl
         << "Core_" << i << "_branch_direct_jump "
         << ooo_cpu[i].total_branch_types[1] << endl
         << "Core_" << i << "_branch_indirect "
         << ooo_cpu[i].total_branch_types[2] << endl
         << "Core_" << i << "_branch_conditional "
         << ooo_cpu[i].total_branch_types[3] << endl
         << "Core_" << i << "_branch_direct_call "
         << ooo_cpu[i].total_branch_types[4] << endl
         << "Core_" << i << "_branch_indirect_call "
         << ooo_cpu[i].total_branch_types[5] << endl
         << "Core_" << i << "_branch_return "
         << ooo_cpu[i].total_branch_types[6] << endl
         << "Core_" << i << "_branch_other " << ooo_cpu[i].total_branch_types[7]
         << endl
         << endl;
...
void print_dram_stats()
{
  ...
  cout << "DRAM_total_data_loads " << uncore.DRAM.stats.data_loads.total_loads
       << endl
       << "DRAM_data_loads_frontal "
       << uncore.DRAM.stats.data_loads.load_cat[FRONTAL] << endl
       << "DRAM_data_loads_dorsal "
       << uncore.DRAM.stats.data_loads.load_cat[DORSAL] << endl
       << "DRAM_data_loads_none " << uncore.DRAM.stats.data_loads.load_cat[NONE]
       << endl
       << endl
       << "DRAM_pseudo_direct_dram_prefetch_reduced_lat "
       << uncore.DRAM.stats.pseudo_direct_dram_prefetch.reduced_lat << endl
       << "DRAM_pseudo_direct_dram_prefetch_zero_lat "
       << uncore.DRAM.stats.pseudo_direct_dram_prefetch.zero_lat << endl
       << endl;

  // DDRP stats
  cout << "DRAM_DDRP_ddrp_req_total " << uncore.DRAM.stats.ddrp.ddrp_req.total
       << endl
       << "DRAM_DDRP_ddrp_req_went_to_dram "
       << uncore.DRAM.stats.ddrp.ddrp_req.went_to_dram << endl
       << "DRAM_DDRP_ddrp_req_RQ_hit_on_ddrp_req "
       << uncore.DRAM.stats.ddrp.ddrp_req.rq_hit[0] << endl
       << "DRAM_DDRP_ddrp_req_RQ_hit_on_llc_miss "
  ...
  for (uint32_t type = 0; type < NUM_TYPES; ++type) {
    cout << "DRAM_DDRP_llc_miss_" << type_names[type] << "_total "
         << uncore.DRAM.stats.ddrp.llc_miss.total[type] << endl
         << "DRAM_DDRP_llc_miss_" << type_names[type] << "_went_to_dram "
         << uncore.DRAM.stats.ddrp.llc_miss.went_to_dram[type] << endl
         << "DRAM_DDRP_llc_miss_" << type_names[type] << "_RQ_hit "
         << uncore.DRAM.stats.ddrp.llc_miss.rq_hit[type] << endl
         << "DRAM_DDRP_llc_miss_" << type_names[type] << "_ddrp_buffer_hit "
         << uncore.DRAM.stats.ddrp.llc_miss.ddrp_buffer_hit[type] << endl
         << endl;
  }

  cout << "DRAM_processed_request_returned_LOAD "
       << uncore.DRAM.stats.dram_process.returned[LOAD] << endl
       << "DRAM_processed_request_returned_RFO "
       << uncore.DRAM.stats.dram_process.returned[RFO] << endl
       << "DRAM_processed_request_returned_PREFETCH "
       << uncore.DRAM.stats.dram_process.returned[PREFETCH] << endl
       << "DRAM_processed_request_returned_WRITEBACK "
       << uncore.DRAM.stats.dram_process.returned[WRITEBACK] << endl
       << "DRAM_processed_request_buffered "
       << uncore.DRAM.stats.dram_process.buffered << endl
       << "DRAM_processed_request_not_returned "
       << uncore.DRAM.stats.dram_process.not_returned << endl
       << endl;

  cout << "ddrp_buffer_insert_called "
       << uncore.DRAM.stats.ddrp_buffer.insert.called << endl
       << "ddrp_buffer_insert_hit " << uncore.DRAM.stats.ddrp_buffer.insert.hit
       << endl
       << "ddrp_buffer_insert_evict "
       << uncore.DRAM.stats.ddrp_buffer.insert.evict << endl
       << "ddrp_buffer_insert_insert "
       << uncore.DRAM.stats.ddrp_buffer.insert.insert << endl
       << "ddrp_buffer_lookup_called "
       << uncore.DRAM.stats.ddrp_buffer.lookup.called << endl
       << "ddrp_buffer_lookup_hit " << uncore.DRAM.stats.ddrp_buffer.lookup.hit
       << endl
       << "ddrp_buffer_lookup_miss "
       << uncore.DRAM.stats.ddrp_buffer.lookup.miss << endl
       << endl;
...
int main(int argc, char **argv)
{
  ...
  DRAM_DBUS_RETURN_TIME =
      (BLOCK_SIZE / DRAM_CHANNEL_WIDTH) * (1.0 * CPU_FREQ / DRAM_MTPS);
  DRAM_DBUS_MAX_CAS =
      DRAM_CHANNELS * (knob::measure_dram_bw_epoch / DRAM_DBUS_RETURN_TIME);
  ...
  for (int i = 0; i < NUM_CPUS; i++) {
    ...
    if (knob::l2c_dump_access_trace) {
      ooo_cpu[i].L2C.tracer.init_tracing(knob::l2c_access_trace_filename,
                                         knob::l2c_dump_access_trace_type, i);
    }
    ooo_cpu[i].L2C.init_rand_engine(champsim_seed,
                                    knob::l2c_pseudo_perfect_prob);
  ...
  if (knob::llc_dump_access_trace) {
    uncore.LLC.tracer.init_tracing(knob::llc_access_trace_filename,
                                   knob::llc_dump_access_trace_type, -1);
  }
  ...
  // Invariant: the off-chip predictor is owned by exactly one side — either
  // every core (core mode) or the LLC (uncore mode), never both and never
  // neither.
  {
    bool core_has = (ooo_cpu[0].offchip_pred != NULL);
    for (uint32_t i = 0; i < NUM_CPUS; ++i) {
      assert((ooo_cpu[i].offchip_pred != NULL) ==
             core_has);  // all cores consistent
    }
    assert(core_has !=
  ...
  while (run_simulation) {
    ...
    if (knob::measure_dram_bw &&
      ...
      uint64_t this_epoch_enqueue_count =
          uncore.DRAM.rq_enqueue_count - uncore.DRAM.last_enqueue_count;
      uncore.DRAM.epoch_enqueue_count =
          (uncore.DRAM.epoch_enqueue_count / 2) + this_epoch_enqueue_count;
      uint32_t quartile =
          ((float)100 * uncore.DRAM.epoch_enqueue_count) / DRAM_DBUS_MAX_CAS;
      if (quartile <= 25) {
        uncore.DRAM.bw = 0;
      } else if (quartile <= 50) {
        uncore.DRAM.bw = 1;
      ...
      uncore.DRAM.next_bw_measure_cycle =
          uncore.cycle + knob::measure_dram_bw_epoch;
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    cout << "Core_" << i << "_cumulative_IPC "
         << ((float)ooo_cpu[i].finish_sim_instr / ooo_cpu[i].finish_sim_cycle)
         << endl
         << "Core_" << i << "_total_instructions "
         << ooo_cpu[i].finish_sim_instr << endl
