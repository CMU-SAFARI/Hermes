...
{
  cout << "ocp_hmp_local_history_length " << knob::ocp_hmp_local_history_length
       << endl
       << "ocp_hmp_local_lhr_size " << knob::ocp_hmp_local_lhr_size << endl
       << "ocp_hmp_local_lhr_index_hash_type "
       << knob::ocp_hmp_local_lhr_index_hash_type << endl
       << endl;
}
...
void OffchipPredHMPLocal::dump_stats()
{
  cout << "ocp_hmp_local_predict_called " << stats.predict.called << endl
       ...
       << "ocp_hmp_local_train_strong_nt_to_weak_nt "
       << stats.train.strong_nt_to_weak_nt << endl
       << "ocp_hmp_local_train_strong_nt_to_strong_nt "
       << stats.train.strong_nt_to_strong_nt << endl
       << "ocp_hmp_local_train_weak_nt_to_weak_t "
       << stats.train.weak_nt_to_weak_t << endl
       << "ocp_hmp_local_train_weak_nt_to_strong_nt "
       << stats.train.weak_nt_to_strong_nt << endl
       << "ocp_hmp_local_train_weak_t_to_strong_t "
       << stats.train.weak_t_to_strong_t << endl
       << "ocp_hmp_local_train_weak_t_to_weak_nt "
       << stats.train.weak_t_to_weak_nt << endl
       << "ocp_hmp_local_train_strong_t_to_strong_t "
       << stats.train.strong_t_to_strong_t << endl
       << "ocp_hmp_local_train_strong_t_to_weak_t "
       << stats.train.strong_t_to_weak_t << endl
       << endl;
}
...
OffchipPredHMPLocal::OffchipPredHMPLocal(uint32_t _cpu, string _type,
                                         uint64_t _seed)
...
bool OffchipPredHMPLocal::predict(ooo_model_instr *arch_instr,
                                  uint32_t data_index, LSQ_ENTRY *lq_entry)
{
  ...
  MYLOG(warmup_complete[cpu],
        "pc %lx lhr_index %d local_history %x conf_state %d", pc, lhr_index,
        local_history, conf_state);

  return prediction;
}

void OffchipPredHMPLocal::train(ooo_model_instr *arch_instr,
                                uint32_t data_index, LSQ_ENTRY *lq_entry)
{
  stats.train.called++;
  ...
  if (old_conf == STRONG_NT) {
    ...
    cout << "Invalid confidence found at PHT index " << old_local_history
         << endl;
    assert(false);
  }
  ...
  new_local_history =
      new_local_history & ((1u << knob::ocp_hmp_local_history_length) - 1);
  ...
  MYLOG(warmup_complete[cpu],
        ...
        pc, lhr_index, +went_offchip, old_conf, new_conf, old_local_history,
        new_local_history);
}
...
uint32_t OffchipPredHMPLocal::get_index(uint64_t pc)
{
  ...
  uint32_t hash =
      HashZoo::getHash(knob::ocp_hmp_local_lhr_index_hash_type, pc_folded_xor);
