  ...
  while (continue_reading) {
    ...
    } else  // not a cloudsuite trace
    {
      ...
      } else  // successfully read the trace
      {
        ...
        // The first 64 bytes of input_instr_v2 are layout-identical to
        // input_instr.
        if (knob::trace_version == 2) {
          ...
          for (int i = 0; i < NUM_INSTR_DESTINATIONS; i++) {
            trace_read_instr.destination_registers[i] =
                trace_read_instr_v2.destination_registers[i];
            trace_read_instr.destination_memory[i] =
                trace_read_instr_v2.destination_memory[i];
          }
          for (int i = 0; i < NUM_INSTR_SOURCES; i++) {
            trace_read_instr.source_registers[i] =
                trace_read_instr_v2.source_registers[i];
            trace_read_instr.source_memory[i] =
                trace_read_instr_v2.source_memory[i];
          }
        }
        ...
        for (int i = 0; i < NUM_INSTR_SOURCES; i++) {
          ...
          /*
                if((!arch_instr.is_branch) && (arch_instr.source_registers[i] >
             25) && (arch_instr.source_registers[i] < 28))
                {
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index)
{
  ...
  LQ.entry[lq_index].rob_part_type =
      rob_pos_get_part_type(LQ.entry[lq_index].rob_position);
  ...
  if (offchip_pred && !knob::offchip_pred_location.compare("core")) {
    LQ.entry[lq_index].went_offchip_pred = offchip_pred->predict(
        &ROB.entry[rob_index], data_index, &LQ.entry[lq_index]);
  }
  LQ.occupancy++;
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index)
{
  ...
  SQ.entry[sq_index].rob_part_type =
      rob_pos_get_part_type(SQ.entry[sq_index].rob_position);
...
void O3_CPU::complete_execution(uint32_t rob_index)
{
  ...
  {
    if (ROB.entry[rob_index].num_mem_ops ==
        0)  // all source memory operands are ready
    {
      if ((ROB.entry[rob_index].executed == INFLIGHT) &&
          (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu])) {
        ROB.entry[rob_index].executed = COMPLETED;
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
  ...
  if (is_it_tlb) {
    ...
    queue->remove_queue(&queue->entry[index],
                        current_core_cycle[queue->entry[index].cpu]);
  } else {
  ...
  ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE)
  {
    ...
    if (knob::offchip_pred_mark_merged_load) {
      LQ.entry[merged].went_offchip =
          LQ.entry[provider->lq_index]
              .went_offchip;  // if provider went offchip, mark merged LQ entry
                              // offchip
    }
...
uint32_t O3_CPU::count_dependency(uint32_t               rob_index,
                                  std::vector<uint32_t> &cat_dependents)
{
  std::bitset<ROB_SIZE> dependent_rob_indices;
  // cout << "[BEGIN tracking dependency for ROB " << rob_index << "]" << endl;
  count_dependency_recursive(rob_index, dependent_rob_indices, cat_dependents,
                             0);
...
void O3_CPU::count_dependency_recursive(
    uint32_t rob_index, std::bitset<ROB_SIZE> &dependent_rob_indices,
    std::vector<uint32_t> &cat_dependents, uint32_t level)
{
  if (knob::load_hit_dependency_max_level &&
      level == knob::load_hit_dependency_max_level) {
    return;  // bound the recusion depth
  }

  TYPE     dependents[ROB_SIZE + 1];
  uint32_t count = ROB.entry[rob_index].registers_instrs_depend_on_me.expand(
      dependents, ROB_SIZE);
  if (count == 0) {
    return;
  }
  ...
  ITERATE_SET(i, ROB.entry[rob_index].registers_instrs_depend_on_me, ROB_SIZE)
  {
    if (!dependent_rob_indices.test(i)) {
      ...
      count_dependency_recursive(i, dependent_rob_indices, cat_dependents,
                                 level + 1);
    }
  }
}
...
void O3_CPU::measure_pipeline_bubble_stats(uint32_t lq_index,
                                           uint32_t rob_index)
{
  stats.bubble.called++;
  ...
  if (LQ.entry[lq_index].went_offchip) {
    ...
    uint64_t bubbles =
        (ROB.entry[rob_index].rob_head_cycle != 0)
            ? (current_core_cycle[cpu] - ROB.entry[rob_index].rob_head_cycle)
            : 0;
...
void O3_CPU::monitor_loads(uint32_t lq_index)
{
  ...
  if (it != load_per_ip_stats.end()) {
    ...
    load_per_ip_stats.insert(
        std::pair<uint64_t, load_per_ip_info_t>(load_ip, info));
  }
  ...
  if (rob_part_type == FRONTAL) {
    ...
    if (it2 != frontal_load_per_ip_stats.end()) {
      ...
      frontal_load_per_ip_stats.insert(
          std::pair<uint64_t, load_per_ip_info_t>(load_ip, info));
    }
  }
}
...
{
  for (uint32_t index = 0; index < knob::rob_frontal_partition_ids.size();
       ++index) {
...
{
  for (uint32_t index = 0; index < knob::rob_dorsal_partition_ids.size();
       ++index) {
...
uint32_t O3_CPU::get_source_index_from_rob(uint32_t rob_index,
                                           uint32_t lq_index)
{
  ...
  assert(num_sources ==
         1);  // is this true? Can one macro x86 instruction generate more than
              // one memory request, both occupying same LQ entry?
  return source_index;
}
