...
{
  std::stringstream ss;
  ss << "ROB_id: " << rob_index << " ROB_pos: " << rob_position
     << " instr_id: " << instr_id << " is_ins: " << (uint32_t)instruction
     << " is_data: " << (uint32_t)is_data << " addr: " << hex << address << dec
...
void PACKET_QUEUE_BASE::record_queue_stats(PACKET *packet)
{
  ...
  if (packet->enque_cycle[module_type][queue_type] <= 0) {
    cout << "=== FAILED ===" << endl;
    cout << "[" << NAME << "] " << " id: " << packet->id
         << " cpu: " << packet->cpu << " instr_id: " << packet->instr_id;
    cout << " address: " << hex << packet->address
         << " full_addr: " << packet->full_addr << dec;
    cout << " instruction: " << (uint32_t)packet->instruction
         << " is_data: " << (uint32_t)packet->is_data;
    cout << " module_type: " << (int32_t)module_type
         << " queue_type: " << (int32_t)queue_type;
    cout << " enque_time: " << packet->enque_cycle[module_type][queue_type]
         << " deque_time: " << packet->deque_cycle[module_type][queue_type]
         << endl;
    assert(false);
  }
  ...
  uint64_t delay = packet->deque_cycle[module_type][queue_type] -
                   packet->enque_cycle[module_type][queue_type];
  assert(delay >= 0);
...
void PACKET_QUEUE_BASE::dump_stats()
{
  ...
  cout << "Core_" << cpu << "_" << NAME << "_total_requests "
       << tot_requests_seen << endl
       << "Core_" << cpu << "_" << NAME << "_total_queueing_delay "
       << tot_queueing_delay << endl
       << "Core_" << cpu << "_" << NAME << "_avg_queueing_delay "
       << (float)tot_queueing_delay / tot_requests_seen << endl
       << "Core_" << cpu << "_" << NAME << "_total_requests_FRONTAL "
       << stats.requests_seen[FRONTAL] << endl
       << "Core_" << cpu << "_" << NAME << "_total_queueing_delay_FRONTAL "
       << stats.total_queueing_delay[FRONTAL] << endl
       << "Core_" << cpu << "_" << NAME << "_avg_queueing_delay_FRONTAL "
       << (float)stats.total_queueing_delay[FRONTAL] /
              stats.requests_seen[FRONTAL]
       << endl
       << "Core_" << cpu << "_" << NAME << "_total_requests_NONE "
       << stats.requests_seen[NONE] << endl
       << "Core_" << cpu << "_" << NAME << "_total_queueing_delay_NONE "
       << stats.total_queueing_delay[NONE] << endl
       << "Core_" << cpu << "_" << NAME << "_avg_queueing_delay_NONE "
       << (float)stats.total_queueing_delay[NONE] / stats.requests_seen[NONE]
       << endl
       << "Core_" << cpu << "_" << NAME << "_total_requests_DORSAL "
       << stats.requests_seen[DORSAL] << endl
       << "Core_" << cpu << "_" << NAME << "_total_queueing_delay_DORSAL "
       << stats.total_queueing_delay[DORSAL] << endl
       << "Core_" << cpu << "_" << NAME << "_avg_queueing_delay_DORSAL "
       << (float)stats.total_queueing_delay[DORSAL] /
              stats.requests_seen[DORSAL]
       << endl
       << endl;
}
  ...
  for (uint32_t i = 0; i < entry.size(); ++i) {
    if (NAME == "L1D_WQ") {
      if (entry[i].full_addr == packet->full_addr) {
        DP(if (warmup_complete[packet->cpu]) {
          cout << "[" << NAME << "] " << __func__ << " cpu: " << packet->cpu
               << " instr_id: " << packet->instr_id << " same address: " << hex
               << packet->address;
          cout << " full_addr: " << packet->full_addr << dec
               << " by instr_id: " << entry[i].instr_id << " index: " << i;
          cout << " cycle " << packet->event_cycle << endl;
