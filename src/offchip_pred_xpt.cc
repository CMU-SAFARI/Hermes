...
void OffchipPredXPT::print_config()
{
  cout << "ocp_xpt_num_entries " << knob::ocp_xpt_num_entries << endl
       ...
       << "ocp_xpt_offchip_threshold " << knob::ocp_xpt_offchip_threshold
       << endl
       << "ocp_xpt_hash_type " << knob::ocp_xpt_hash_type << endl
       << "ocp_xpt_use_physical_address " << knob::ocp_xpt_use_physical_address
       << endl
...
OffchipPredXPT::OffchipPredXPT(uint32_t _cpu, string _type, uint64_t _seed)
    : OffchipPredBase(_cpu, _type, _seed)
{
  bzero(&stats, sizeof(stats));

  // XPT is natively a physical-page-indexed, LLC-side predictor. At the CORE,
  // predict() runs at add_load_queue() before translation, where the physical
  // address is still 0 — so physical indexing is meaningless and we forbid it
  // there. At the LLC (uncore) the physical address IS available, so physical
  // indexing is allowed; only bar it core-side.
  if (knob::ocp_xpt_use_physical_address &&
      !knob::offchip_pred_location.compare("core")) {
    cerr << "[XPT] ERROR: ocp_xpt_use_physical_address=true, but XPT is "
            "instantiated inside the core."
         << endl
         << "[XPT] The physical address is not available at prediction time "
            "(add_load_queue), so XPT"
         << endl
         << "[XPT] must index by virtual address when placed inside the core. "
            "Set"
         << endl
         << "[XPT] ocp_xpt_use_physical_address=false, or move XPT beside the "
            "LLC to use physical addresses."
         << endl;
    assert(false && "XPT core-side cannot use physical address");
  }

  assert(knob::ocp_xpt_assoc > 0 &&
         knob::ocp_xpt_num_entries >= knob::ocp_xpt_assoc);
...
// Shared prediction logic. `addr` has already been selected by the caller
// (virtual or physical, core- or uncore-side); everything below is
// placement-agnostic.
bool OffchipPredXPT::predict_helper(uint64_t addr)
{
  ...
  auto it = find_if(m_tracker[set].begin(), m_tracker[set].end(),
                    [page](const ocp_xpt_tracker_entry_t &entry) {
  ...
  if (prediction) {
    stats.predict.offchip++;
  } else {
    stats.predict.not_offchip++;
  }

  return prediction;
}

// Shared training logic. `addr` already selected by the caller; `went_offchip`
// is the resolved outcome (LSQ_ENTRY::went_offchip core-side,
// PACKET::went_offchip uncore-side).
...
bool OffchipPredXPT::predict(ooo_model_instr *arch_instr, uint32_t data_index,
                             LSQ_ENTRY *lq_entry)
{
  // XPT is natively physical-page-indexed; core-side it falls back to virtual
  // page (see constructor). The knob selects which address to use.
  uint64_t addr = knob::ocp_xpt_use_physical_address
                      ? lq_entry->physical_address
                      : lq_entry->virtual_address;
  return predict_helper(addr);
}

void OffchipPredXPT::train(ooo_model_instr *arch_instr, uint32_t data_index,
                           LSQ_ENTRY *lq_entry)
{
  uint64_t addr = knob::ocp_xpt_use_physical_address
                      ? lq_entry->physical_address
...
{
  // beside the LLC the physical address is available on the PACKET. (PACKET has
  // no virtual address yet — to be added later to compare virtual vs physical
  // indexing.)
