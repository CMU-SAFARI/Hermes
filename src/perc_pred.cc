...
perceptron_pred_t::perceptron_pred_t(vector<int32_t> _activated_features,
    ...
    : activation_threshold(threshold), max_weight(max_w), min_weight(min_w),
      pos_weight_delta(pos_delta), neg_weight_delta(neg_delta),
...
void perceptron_pred_t::dump_stats()
{
  cout << "perc_predict_called " << stats.predict.called << endl
       ...
       << "perc_train_incr_weight_match " << stats.train.incr_weight_match
       << endl
       << "perc_train_incr_weight_mismatch " << stats.train.incr_weight_mismatch
       << endl
       << "perc_train_decr_weight_match " << stats.train.decr_weight_match
       << endl
       << "perc_train_decr_weight_mismatch " << stats.train.decr_weight_mismatch
       << endl
       << endl;

  for (uint32_t feature = 0; feature < num_features; ++feature) {
    cout << "perc_feature_" << feature_names[activated_features[feature]]
         << "_incr_done " << stats.weight.incr_done[activated_features[feature]]
         << endl
         << "perc_feature_" << feature_names[activated_features[feature]]
         << "_incr_satu " << stats.weight.incr_satu[activated_features[feature]]
         << endl
         << "perc_feature_" << feature_names[activated_features[feature]]
         << "_decr_done " << stats.weight.decr_done[activated_features[feature]]
         << endl
         << "perc_feature_" << feature_names[activated_features[feature]]
         << "_decr_satu " << stats.weight.decr_satu[activated_features[feature]]
         << endl
         << endl;
  }
}
...
void perceptron_pred_t::predict(state_info_t *state, bool &prediction,
                                float &perc_weight_sum)
{
  ...
  for (uint32_t feature = 0; feature < num_features; ++feature) {
    ...
    cummulative_weight +=
        weights[feature]
            .array[weight_indices[feature]];  // sum up all feature weights
  }
...
void perceptron_pred_t::train(state_info_t *state, float perc_weight_sum,
                              bool pred_output, bool true_output)
{
  stats.train.called++;
  MYLOG(warmup_complete[cpu], "==========================");
  MYLOG(warmup_complete[cpu],
        "perc_train %s perc_weight_sum: %f offchip_real: %d offchip_pred: %d",
        state->to_string().c_str(), perc_weight_sum, true_output, pred_output);
  ...
  if (true_output == true) {
    if (pred_output == true_output) {
      ...
      if (perc_weight_sum >= neg_train_thresh &&
          perc_weight_sum <= pos_train_thresh) {
        MYLOG(warmup_complete[cpu],
              "correct_pred::true increasing weights ...");
        incr_weights(weight_indices);
        stats.train.incr_weight_match++;
      } else {
        MYLOG(warmup_complete[cpu],
              "correct_pred::true perc_sum_weight already saturated ...");
      }
    } else {
      // should have predicted true, but actually predicted false
      MYLOG(warmup_complete[cpu],
            "incorr_pred::real=true::pred=false increasing weights ...");
      incr_weights(weight_indices);
    ...
    if (pred_output == true_output) {
      ...
      if (perc_weight_sum >= neg_train_thresh &&
          perc_weight_sum <= pos_train_thresh) {
        MYLOG(warmup_complete[cpu],
              "correct_pred::false decreasing weights ...");
        decr_weights(weight_indices);
        stats.train.decr_weight_match++;
      } else {
        MYLOG(warmup_complete[cpu],
              "correct_pred::false perc_sum_weight already saturated ...");
      }
    } else {
      // should have predicted false, but actually predicted true
      MYLOG(warmup_complete[cpu],
            "incorr_pred::real=false::pred=true decreasing weights ...");
      decr_weights(weight_indices);
...
{
  for (uint32_t feature = 0; feature < num_features; ++feature) {
    if ((weights[feature].array[weight_indices[feature]] + pos_weight_delta) <=
        max_weight) {
      MYLOG(warmup_complete[cpu], "feature %s index %d old_weight %f",
            feature_names[activated_features[feature]].c_str(),
            weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
      weights[feature].array[weight_indices[feature]] += pos_weight_delta;
      MYLOG(warmup_complete[cpu], "feature %s index %d new_weight %f",
            feature_names[activated_features[feature]].c_str(),
            weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
      stats.weight.incr_done[activated_features[feature]]++;
    } else {
      MYLOG(warmup_complete[cpu], "feature %s index %d saturated_weight %f",
            feature_names[activated_features[feature]].c_str(),
            weight_indices[feature],
            weights[feature].array[weight_indices[feature]]);
...
{
  for (uint32_t feature = 0; feature < num_features; ++feature) {
    if ((weights[feature].array[weight_indices[feature]] - neg_weight_delta) >=
        min_weight) {
      MYLOG(warmup_complete[cpu], "feature %s index %d old_weight %f",
            feature_names[activated_features[feature]].c_str(),
            weight_indices[feature],
...
vector<uint32_t>
perceptron_pred_t::generate_indices_from_state(state_info_t *state)
{
