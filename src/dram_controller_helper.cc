...
int MEMORY_CONTROLLER::schedule_FR_FCFS(PACKET_QUEUE *queue,
...
int MEMORY_CONTROLLER::schedule_CR_FCFS(PACKET_QUEUE *queue,
                                        uint8_t      &row_buffer_hit)
{
  int      index        = -1;
  uint64_t oldest_cycle = UINT64_MAX, read_addr;
...
int MEMORY_CONTROLLER::schedule_FRONTAL_FR_FCFS(PACKET_QUEUE *queue,
                                                uint8_t      &row_buffer_hit)
{
  int      sel_index          = -1;
  ...
  for (uint32_t i = 0; i < queue->SIZE; i++) {
    ...
    read_row_buffer_hit =
        (bank_request[read_channel][read_rank][read_bank].open_row == read_row);
...
int MEMORY_CONTROLLER::schedule_FR_FRONTAL_FCFS(PACKET_QUEUE *queue,
                                                uint8_t      &row_buffer_hit)
{
  int      sel_index          = -1;
  bool     sel_is_frontal     = false;
...
int MEMORY_CONTROLLER::schedule_ROB_PART_FR_FCFS(PACKET_QUEUE *queue,
                                                 uint8_t      &row_buffer_hit)
{
  int      sel_index          = -1;
...
int MEMORY_CONTROLLER::schedule_FR_ROB_PART_FCFS(PACKET_QUEUE *queue,
                                                 uint8_t      &row_buffer_hit)
{
  int      sel_index          = -1;
  int8_t   sel_rob_part       = INT8_MAX;
