...
const char *map_ocp_basic_feature_type_string[] = {
    ...
    "PC_Page", "PC_Addr",  "CL_first_access"};
...
void OffchipPredBasic::print_config()
{
  cout << "ocp_basic_table_size " << knob::ocp_basic_table_size << endl
       ...
       << "ocp_basic_include_data_index_type "
       << knob::ocp_basic_include_data_index_type << endl
       << "ocp_basic_pc_buf_size " << knob::ocp_basic_pc_buf_size << endl
       << "ocp_basic_feature_type "
       << ocp_basic_get_feature_string(knob::ocp_basic_feature_type) << endl
...
void OffchipPredBasic::dump_stats()
{
  cout << "ocp_basic_train_called " << stats.train.called << endl
       ...
       << "ocp_basic_predict_offchip_miss " << stats.predict.offchip_miss
       << endl
       ...
       << "ocp_basic_page_buf_lookup_eviction " << stats.page_buf.eviction
       << endl
       << "ocp_basic_page_buf_lookup_insertion " << stats.page_buf.insertion
       << endl
       << endl
...
OffchipPredBasic::OffchipPredBasic(uint32_t _cpu, string _type, uint64_t _seed)
...
bool OffchipPredBasic::predict(ooo_model_instr *arch_instr, uint32_t data_index,
                               LSQ_ENTRY *lq_entry)
{
  ...
  // compute index
  uint32_t index =
      get_hash(load_pc, count, data_index, vaddr, vpage, voffset, first_access);
  ...
  if (m_table[index].total.val() > 0) {
    ...
    if (ratio >= knob::ocp_basic_conf_thresh) {
      ...
      MYLOG(warmup_complete[cpu],
            "load_pc: %x count: %d data_idx: %d ratio: %f predicted: MISS",
            load_pc, count, data_index, ratio);
      return true;
    } else {
      stats.predict.offchip_hit++;
      MYLOG(warmup_complete[cpu],
            "load_pc: %x count: %d data_idx: %d ratio: %f predicted: HIT",
            load_pc, count, data_index, ratio);
      return false;
    }
  } else {
...
void OffchipPredBasic::train(ooo_model_instr *arch_instr, uint32_t data_index,
                             LSQ_ENTRY *lq_entry)
{
  uint64_t load_pc = arch_instr->ip;
  uint64_t vaddr   = lq_entry->virtual_address;
  ...
  MYLOG(warmup_complete[cpu],
        ...
        "miss_counter: %d",
        load_pc, count, data_index, vaddr, vpage, vofffset,
        (uint32_t)lq_entry->went_offchip, (uint32_t)lq_entry->went_offchip_pred,
        index, m_table[index].total.val(), m_table[index].miss.val());
}

uint32_t OffchipPredBasic::get_hash(uint64_t load_pc, uint32_t count,
                                    uint32_t data_index, uint64_t vaddr,
                                    uint64_t vpage, uint32_t voffset,
                                    bool first_access)
{
...
uint32_t OffchipPredBasic::get_hash_pc_count(uint64_t load_pc, uint32_t count)
{
  ...
  return (HashZoo::getHash(knob::ocp_basic_hash_type, folded_xor(raw_data, 2)) %
          knob::ocp_basic_table_size);
}

uint32_t OffchipPredBasic::get_hash_pc_data_index(uint64_t load_pc,
                                                  uint32_t data_index)
{
  uint32_t folded_pc  = folded_xor(load_pc, 2);
...
uint32_t OffchipPredBasic::get_hash_pc_offset(uint64_t load_pc,
                                              uint32_t voffset)
{
  uint64_t raw_data = load_pc;
  raw_data          = raw_data << 6;
...
void OffchipPredBasic::lookup_pc(uint64_t load_pc, uint32_t &count)
{
  ...
  auto                     it = find_if(m_pc_buffer.begin(), m_pc_buffer.end(),
                                        [load_pc](const ocp_basic_pc_buf_entry_t &entry) {
                      return entry.pc == load_pc;
                    });
...
void OffchipPredBasic::lookup_address(uint64_t vaddr, uint64_t vpage,
                                      uint32_t voffset, bool &first_access)
{
  ...
  auto it = find_if(m_page_buffer.begin(), m_page_buffer.end(),
                    [vpage](ocp_basic_page_buf_entry_t &entry) {
