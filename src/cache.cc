...
void print_cache_config()
{
  cout << "itlb_set " << ITLB_SET << endl
       ...
       << "itlb_priority_rq_type "
       << priority_name_string[knob::itlb_priority_rq_priority_type] << endl
       << endl
       ...
       << "dtlb_priority_rq_type "
       << priority_name_string[knob::dtlb_priority_rq_priority_type] << endl
       << endl
       ...
       << "stlb_priority_rq_type "
       << priority_name_string[knob::stlb_priority_rq_priority_type] << endl
       << endl
       ...
       << "l1i_priority_rq_type "
       << priority_name_string[knob::l1i_priority_rq_priority_type] << endl
       << endl
       ...
       << "l1d_priority_rq_type "
       << priority_name_string[knob::l1d_priority_rq_priority_type] << endl
       << endl
       ...
       << "l2c_priority_rq_type "
       << priority_name_string[knob::l2c_priority_rq_priority_type] << endl
       << endl
...
void CACHE::create_rq()
{
  ...
  if (priority_rq) {
    ...
    cout << "Adding priority-RQ type " << priority_type << " in " << NAME
         << endl;
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (do_fill) {
      ...
      if (cache_type == IS_LLC &&
        ...
        ooo_cpu[fill_cpu].offchip_predictor_track_llc_eviction(
            set, way, block[set][way].full_addr);
      }
...
void CACHE::handle_writeback()
{
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) &&
    ...
    {
      WQ.entry[index].hit_where =
          assign_hit_where(cache_type, 0);  // writeback hit
      ...
      if (cache_type == IS_L1D)  // RFO miss
      {
                   ...
                   (MSHR.occupancy < MSHR_SIZE))  // this is a new miss
        {
          if (cache_type == IS_LLC) {
            ...
            // check to make sure the DRAM RQ has room for this LLC RFO miss
            if (lower_level &&
                lower_level->get_occupancy(1, WQ.entry[index].address) ==
                    lower_level->get_size(1, WQ.entry[index].address)) {
              miss_handled = 0;
            } else {
          ...
          {
            WQ.entry[index].hit_where =
                assign_hit_where(cache_type, 3);  // writeback hit in MSHR
...
void CACHE::handle_read()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    // handle the oldest entry
    if ((RQ->peek().event_cycle <= current_core_cycle[read_cpu]) &&
        (RQ->occupancy > 0)) {
      ...
      if (way >= 0)  // read hit
      {
        ...
        if (rq_entry.type == LOAD) {
          if (cache_type == IS_L1I) {
            l1i_prefetcher_cache_operate(read_cpu, rq_entry.ip, 1,
                                         block[set][way].prefetch);
          }
          if (cache_type == IS_L1D) {
            l1d_prefetcher_operate(rq_entry.full_addr, rq_entry.ip, 1,
                                   rq_entry.type);
          } else if (cache_type == IS_L2C) {
            l2c_prefetcher_operate(block[set][way].address << LOG2_BLOCK_SIZE,
                                   rq_entry.ip, 1, rq_entry.type, 0);
        ...
        // update replacement policy
        if (cache_type == IS_LLC) {
          llc_update_replacement_state(read_cpu, set, way,
                                       block[set][way].full_addr, rq_entry.ip,
                                       0, rq_entry.type, 1);
        } else {
          update_replacement_state(read_cpu, set, way,
                                   block[set][way].full_addr, rq_entry.ip, 0,
                                   rq_entry.type, 1);
        }
        ...
        if (rq_entry.is_data && rq_entry.type == LOAD) {
          block[set][way]
              .reuse_frontal_dorsal[ooo_cpu[read_cpu].rob_pos_get_part_type(
                  rq_entry.rob_position)]++;
        }
        if (knob::track_load_hit_dependency_in_cache) {
          vector<uint32_t> cat_dependents;
          cat_dependents.resize(DEP_INSTR_TYPES, 0);
          block[set][way].dependents += ooo_cpu[read_cpu].count_dependency(
              rq_entry.rob_index, cat_dependents);
        ...
        uint64_t deque_cycle =
            cache_type == IS_LLC ? uncore.cycle : current_core_cycle[read_cpu];
      ...
      } else  // read miss
      {
        DP(if (warmup_complete[read_cpu]) {
          ...
          cout << " instr_id: " << rq_entry.instr_id << " address: " << hex
               << rq_entry.address;
                   ...
                   (MSHR.occupancy < MSHR_SIZE))  // this is a new miss
        {
          if (cache_type == IS_LLC) {
            ...
            if (lower_level->get_occupancy(1, rq_entry.address) ==
                lower_level->get_size(1, rq_entry.address)) {
              miss_handled = 0;
            } else {
              ...
              if (lower_level) {
                ...
                if (rq_entry.is_data && rq_entry.type == LOAD) {
                  if (rq_entry.rob_position < 0 ||
                      rq_entry.rob_position >= ROB_SIZE) {
                    cout << "invalid ROB position: index: " << index
                         << " pos: " << rq_entry.rob_position << endl;
            ...
            } else  // this is the last level
            {
              if (cache_type == IS_STLB) {
                ...
                uint64_t pa          = va_to_pa(read_cpu, rq_entry.instr_id,
                                                rq_entry.full_addr, rq_entry.address, 0);
          ...
          } else if (mshr_index != -1)  // already in-flight miss
          {
            ...
            if (rq_entry.type == RFO) {
              if (rq_entry.tlb_access) {
                ...
                MSHR.entry[mshr_index].sq_index_depend_on_me.join(
                    rq_entry.sq_index_depend_on_me, SQ_SIZE);
              }
              if (rq_entry.load_merged) {
                ...
                MSHR.entry[mshr_index].lq_index_depend_on_me.join(
                    rq_entry.lq_index_depend_on_me, LQ_SIZE);
              }
            } else {
              if (rq_entry.instruction) {
                ...
                DP(if (warmup_complete[read_cpu]) {
                  ...
                  cout << " merged rob_index: " << rq_entry.rob_index
                       << " instr_id: " << rq_entry.instr_id
                       << " lq_index: " << rq_entry.lq_index << endl;
                });
            ...
            DP(if (warmup_complete[read_cpu]) {
              cout << "[" << NAME << "] " << __func__ << " mshr merged";
              cout << " instr_id: " << rq_entry.instr_id
                   << " prior_id: " << MSHR.entry[mshr_index].instr_id;
        ...
        if (miss_handled) {
          ...
          if (rq_entry.type == LOAD) {
            ...
            if (cache_type == IS_L1D) {
              l1d_prefetcher_operate(rq_entry.full_addr, rq_entry.ip, 0,
                                     rq_entry.type);
            }
            if (cache_type == IS_L2C) {
              l2c_prefetcher_operate(rq_entry.address << LOG2_BLOCK_SIZE,
                                     rq_entry.ip, 0, rq_entry.type, 0);
            }
            if (cache_type == IS_LLC) {
              cpu = read_cpu;
              llc_prefetcher_operate(rq_entry.address << LOG2_BLOCK_SIZE,
                                     rq_entry.ip, 0, rq_entry.type, 0);
              cpu = 0;
            }
          }
...
void CACHE::handle_prefetch()
{
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) &&
      ...
      {
        PQ.entry[index].hit_where =
            assign_hit_where(cache_type, 0);  // prefetch hit
      ...
      } else  // prefetch miss
      {
                   ...
                   (MSHR.occupancy < MSHR_SIZE))  // this is a new miss
        {
          ...
          if (lower_level) {
            if (cache_type == IS_LLC) {
              ...
              if (lower_level->get_occupancy(3, PQ.entry[index].address) ==
                ...
                lower_level->add_pq(
                    &PQ.entry[index]);  // add it to the PQ of lower cache
              }
            }
          }
        } else {
          ...
          {
            PQ.entry[index].hit_where =
                assign_hit_where(cache_type, 3);  // prefetch hit in MSHR
...
int CACHE::check_hit(PACKET *packet)
{
  ...
  if (packet->is_data &&
      packet->type == LOAD  // this should only be modeled for data loads
    ...
    if ((knob::llc_pseudo_perfect_enable_frontal &&
         ooo_cpu[packet->cpu].rob_pos_is_frontal(packet->rob_position)) ||
        (knob::llc_pseudo_perfect_enable_dorsal &&
         ooo_cpu[packet->cpu].rob_pos_is_dorsal(packet->rob_position))) {
  ...
  if (packet->is_data &&
    ...
    if ((knob::l2c_pseudo_perfect_enable_frontal &&
         ooo_cpu[packet->cpu].rob_pos_is_frontal(packet->rob_position)) ||
        (knob::l2c_pseudo_perfect_enable_dorsal &&
         ooo_cpu[packet->cpu].rob_pos_is_dorsal(packet->rob_position))) {
      stats.pseudo_perfect.data_load_miss_eligible_for_pseudo_hit_promotion++;
...
int CACHE::add_rq(PACKET *packet)
{
      ...
      if (packet->type == RFO) {
        ...
        DP(if (warmup_complete[packet->cpu]) {
          cout << "[RFO merging in RQ] cache_type " << cache_type
               << " incoming_type " << packet->type << " exsisting_type "
               << rq_entry.type << " incoming_ROB_index " << packet->rob_index
               << " exsisting_ROB_index " << rq_entry.rob_index
               << " incoming_ROB_pos " << packet->rob_position
  ...
  uint64_t enque_cycle =
      cache_type == IS_LLC ? uncore.cycle : current_core_cycle[packet->cpu];
  ...
  DP(if (warmup_complete[rq_entry.cpu]) {
    cout << "[" << NAME << "_RQ] " << __func__
         << " instr_id: " << rq_entry.instr_id << " address: " << hex
         << rq_entry.address;
    cout << " full_addr: " << rq_entry.full_addr << dec;
    cout << " type: " << +rq_entry.type << " head: " << RQ->get_head()
         << " tail: " << RQ->get_tail() << " occupancy: " << RQ->occupancy;
    cout << " event: " << rq_entry.event_cycle
         << " current: " << current_core_cycle[rq_entry.cpu] << endl;
  });
...
int CACHE::add_pq(PACKET *packet)
{
  ...
  if (wq_index != -1) {
    packet->hit_where =
        assign_hit_where(cache_type, 2);  // prefetch hitting in WQ
...
void CACHE::prefetcher_feedback(uint64_t &pref_gen, uint64_t &pref_fill,
                                uint64_t &pref_used, uint64_t &pref_late)
{
  pref_gen  = pf_issued;
  pref_fill = pf_filled;
  pref_used = pf_useful;
...
void CACHE::track_stats_from_victim(uint32_t set, uint32_t way)
{
  ...
  if (knob::track_load_hit_dependency_in_cache &&
    ...
    uint32_t dep_branch_mispred =
        block[set][way].cat_dependents[DEP_INSTR_BRANCH_MISPRED];
    ...
    uint32_t dep_branch =
        block[set][way].cat_dependents[DEP_INSTR_BRANCH_MISPRED] +
        block[set][way].cat_dependents[DEP_INSTR_BRANCH_CORRECT];
...
void CACHE::send_signal_to_core(uint32_t cpu, PACKET packet)
{
  ...
  if (ooo_cpu[cpu].LQ.entry[lq_index].rob_index != rob_index) {
    cout << "rob_index in LQ entry does not match with the rob_index from RQ "
            "entry"
...
void CACHE::handle_prefetch_feedback()
{
  ...
  if (knob::measure_cache_acc && cycle >= next_measure_cycle) {
    this_epoch_accuracy =
        pf_filled_epoch ? 100 * (float)pf_useful_epoch / pf_filled_epoch : 0;
