...
{
  cout << "ocp_ttp_catalog_cache_sets " << knob::ocp_ttp_catalog_cache_sets
       << endl
       << "ocp_ttp_catalog_cache_assoc " << knob::ocp_ttp_catalog_cache_assoc
       << endl
       << "ocp_ttp_hash_type " << knob::ocp_ttp_hash_type << endl
       << "ocp_ttp_partial_tag_size " << knob::ocp_ttp_partial_tag_size << endl
       << "ocp_ttp_enable_track_llc_eviction "
       << knob::ocp_ttp_enable_track_llc_eviction << endl
       << endl;
}
...
void OffchipPredTTP::dump_stats()
{
  cout << "ocp_ttp_train_called " << stats.train.called << endl
       ...
       << "ocp_ttp_train_not_offchip_and_in_catalog "
       << stats.train.not_offchip_and_in_catalog << endl
       << "ocp_ttp_train_not_offchip_and_added_in_catalog "
       << stats.train.not_offchip_and_added_in_catalog << endl
       << "ocp_ttp_train_went_offchip " << stats.train.went_offchip << endl
       << "ocp_ttp_train_went_offchip_and_not_in_catalog "
       << stats.train.went_offchip_and_not_in_catalog << endl
       << "ocp_ttp_train_went_offchip_and_deleted_from_catalog "
       << stats.train.went_offchip_and_deleted_from_catalog << endl
       << endl
       << "ocp_ttp_llc_eviction_called " << stats.llc_eviction.called << endl
       << "ocp_ttp_llc_eviction_physical_tag_not_found "
       << stats.llc_eviction.physical_tag_not_found << endl
       << "ocp_ttp_llc_eviction_deleted_from_catalog_cache "
       << stats.llc_eviction.deleted_from_catalog_cache << endl
       << "ocp_ttp_llc_eviction_found_in_rev_index_not_found_in_catalog "
       << stats.llc_eviction.found_in_rev_index_not_found_in_catalog << endl
       << endl;
}
...
OffchipPredTTP::OffchipPredTTP(uint32_t _cpu, string _type, uint64_t _seed)
...
void OffchipPredTTP::train(ooo_model_instr *arch_instr, uint32_t data_index,
                           LSQ_ENTRY *lq_entry)
{
  stats.train.called++;
...
bool OffchipPredTTP::predict(ooo_model_instr *arch_instr, uint32_t data_index,
                             LSQ_ENTRY *lq_entry)
{
  uint64_t vcladdr = lq_entry->virtual_address >> LOG2_BLOCK_SIZE;
...
void OffchipPredTTP::update_catalog_cache(uint64_t vaddr, uint64_t paddr,
                                          bool went_offchip)
{
  ...
  auto     it  = find_if(catalog_cache[set].begin(), catalog_cache[set].end(),
                         [v_partial_tag](pair<uint32_t, uint32_t> p) {
                      return p.first == v_partial_tag;
                    });
  ...
  if (!went_offchip)  // if hits in cache hierarchy
  {
    ...
    if (it != catalog_cache[set].end()) {
      ...
      catalog_cache[set].push_back(
          pair<uint32_t, uint32_t>(v_partial_tag, p_partial_tag));
...
bool OffchipPredTTP::lookup_catalog_cache(uint64_t addr)
{
  ...
  auto     it = find_if(catalog_cache[set].begin(), catalog_cache[set].end(),
                        [partial_tag](pair<uint32_t, uint32_t> p) {
                      return p.first == partial_tag;
                    });
  return (it != catalog_cache[set].end()) ? true : false;
}

void OffchipPredTTP::add_to_catalog_cache_rev_index(uint32_t p_partial_tag,
                                                    uint32_t set)
{
  ...
  if (it == catalog_cache_rev_index.end()) {
    catalog_cache_rev_index.insert(
        pair<uint32_t, uint32_t>(p_partial_tag, set));
  }
...
void OffchipPredTTP::track_llc_eviction(uint64_t paddr)
{
   ...
   */
  auto it2 = find_if(catalog_cache[set].begin(), catalog_cache[set].end(),
                     [p_partial_tag](pair<uint32_t, uint32_t> p) {
                       return p.second == p_partial_tag;
                     });
