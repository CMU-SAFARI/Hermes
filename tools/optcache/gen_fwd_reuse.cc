...
int main(int argc, char **argv)
{
  ...
  string out_filename                = string(argv[2]) + ".reuse.zst";

  ZstdReader infile;
  infile.open(cache_access_trace_filename);
  ...
  while (infile.read(&address, sizeof(uint64_t)) &&
         infile.read(&type, sizeof(uint8_t)) &&
         infile.read(&hit, sizeof(bool))) {
  ...
  for (uint32_t index = 0; index < reuse_dist.size(); ++index) {
    ...
    outfile.write(&reuse, sizeof(uint64_t));
