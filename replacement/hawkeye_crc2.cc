...
uint32_t HawkeyeCRC2Repl::find_victim(uint32_t cpu, uint64_t instr_id,
...
void HawkeyeCRC2Repl::update_addr_history_lru(unsigned int sampler_set,
                                              unsigned int curr_lru)
{
  for (map<uint64_t, ADDR_INFO>::iterator it =
           addr_history[sampler_set].begin();
       it != addr_history[sampler_set].end(); it++) {
...
void HawkeyeCRC2Repl::update_replacement_state(uint32_t cpu, uint32_t set,
