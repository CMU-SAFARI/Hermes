...
void SHiPRepl::initialize_replacement()
{
  ...
  // are reproducible: srand(time(NULL)) made SHiP (and thus IPC) vary
  // run-to-run.
  srand(champsim_seed);
...
// update sampler
void SHiPRepl::update_sampler(uint32_t cpu, uint32_t s_idx, uint64_t address,
                              uint64_t ip, uint8_t type)
{
  ...
  for (match = 0; match < SAMPLER_WAY; match++) {
    if (s_set[match].valid && (s_set[match].tag == tag)) {
      ...
      /*
            if (draw_transition)
                printf("cycle: %lu SHCT: %d ip: 0x%llX SAMPLER_HIT cl_addr:
  ...
  if (match == SAMPLER_WAY) {
    for (match = 0; match < SAMPLER_WAY; match++) {
      if (s_set[match].lru ==
          (SAMPLER_WAY - 1))  // Sampler uses LRU replacement
      {
        if (s_set[match].used == 0) {
          ...
          /*
                    if (draw_transition)
                        printf("cycle: %lu SHCT: %d ip: 0x%llX SAMPLER_MISS
             cl_addr: 0x%llX page: 0x%llX block: %ld set: %d\n",
                        ooo_cpu[cpu].current_cycle, SHCT[cpu][SHCT_idx].dead,
             s_set[match].ip, address>>6, address>>12, (address>>6) & 0x3F,
             s_idx);
                    */
        }
...
uint32_t SHiPRepl::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set,
...
void SHiPRepl::update_replacement_state(uint32_t cpu, uint32_t set,
