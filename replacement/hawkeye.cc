...
{
  cout << "hawkeye_pred_counter_width " << knob::hawkeye_pred_counter_width
       << endl
       ...
       << "hawkeye_optgen_hist_len_factor "
       << knob::hawkeye_optgen_hist_len_factor << endl
       << endl;
}
...
void HawkeyeRepl::initialize_replacement()
{
  ...
  for (uint32_t set = 0; set < LLC_SET; ++set) {
    metadata[set] =
        (HawkeyeMetadata **)calloc(LLC_WAY, sizeof(HawkeyeMetadata *));
    assert(metadata[set]);
...
void HawkeyeRepl::update_replacement_state(uint32_t cpu, uint32_t set,
...
uint32_t HawkeyeRepl::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set,
...
void HawkeyeRepl::dump_stats()
{
  ...
  cout << "hawkeye.repl.update_repl_state.called "
       << stats.update_repl_state.called << endl
       << "hawkeye.repl.update_repl_state.cache_friendly "
       << stats.update_repl_state.cache_friendly << endl
       << "hawkeye.repl.update_repl_state.cache_friendly_hit "
       << stats.update_repl_state.cache_friendly_hit << endl
       << "hawkeye.repl.update_repl_state.cache_friendly_miss "
       << stats.update_repl_state.cache_friendly_miss << endl
       << "hawkeye.repl.update_repl_state.cache_adverse "
       << stats.update_repl_state.cache_adverse << endl
       << "hawkeye.repl.update_repl_state.cache_adverse_hit "
       << stats.update_repl_state.cache_adverse_hit << endl
       << "hawkeye.repl.update_repl_state.cache_adverse_miss "
       ...
       << stats.find_victim.max_rrip_not_found << endl
       << endl;
}
...
bool HawkeyePred::predict(uint64_t ip)
{
  ...
  uint32_t index = gen_index(ip);
  if (confidence[index]->val() >
      confidence[index]->max() / 2)  // predict cache-friendly
  {
...
{
  return (HashZoo::getHash(knob::hawkeye_pred_hash_type, folded_xor(ip, 2)) %
          knob::hawkeye_pred_size);
}
...
void HawkeyePred::dump_stats()
{
  cout << "hawkeye.predictor.train.called " << stats.train.called << endl
       ...
       << "hawkeye.predictor.predict.cache_friendly "
       << stats.predict.cache_friendly << endl
       << "hawkeye.predictor.predict.cache_adverse "
       << stats.predict.cache_adverse << endl
       << endl;
}
...
bool OPTgen::update_access(uint64_t address, uint32_t set)
{
  ...
  access_history[set].push_back(
      pair<uint64_t, uint64_t>(ca_addr, timestamp[set]));
  ...
  if (prev_occurence == -1)  // this is the first access
  {
    ...
    bool opt_miss = false;
    for (int32_t index = occupancy_vector[set].size() - 2;
         index >= prev_occurence; --index) {
...
void OPTgen::dump_stats()
{
  cout << "hawkeye.OPTgen.update.called " << stats.update.called << endl
       << "hawkeye.OPTgen.update.access_history_spill "
       << stats.update.access_history_spill << endl
       << "hawkeye.OPTgen.update.occupancy_vector_spill "
       << stats.update.occupancy_vector_spill << endl
       << "hawkeye.OPTgen.update.first_access " << stats.update.first_access
       << endl
       << "hawkeye.OPTgen.update.reused_access " << stats.update.reused_access
       << endl
