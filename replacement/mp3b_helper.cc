...
static feature_spec default_single_1_specs[] = {
    {F_OFF, 15, 1, 6, 0, 1},  {F_PC, 7, 14, 43, 11, 0},
    {F_PC, 16, 3, 11, 16, 1}, {F_INS, 16, 0, 0, 0, 1},
    {F_OFF, 10, 0, 6, 0, 1},  {F_PC, 10, 1, 53, 10, 0},
    {F_BIAS, 16, 0, 0, 0, 0}, {F_INS, 8, 0, 0, 0, 1},
    {F_PC, 17, 6, 20, 0, 1},  {F_BURST, 6, 11, 22, 9, 0},
    {F_LM, 9, 0, 0, 0, 0},    {F_PC, 17, 6, 20, 0, 1},
    {F_INS, 16, 2, 47, 2, 0}, {F_INS, 17, 0, 0, 0, 1},
    {F_PC, 16, 8, 16, 5, 0},  {F_PC, 17, 6, 20, 14, 1},
};

// this is used in multi-core
static feature_spec default_multi_3_specs[] = {
    {F_BIAS, 1, 0, 0, 0, 0},  {F_PC, 16, 9, 25, 9, 1},
    {F_INS, 8, 4, 8, 7, 2},   {F_PC, 6, 9, 28, 12, 1},
    {F_OFF, 14, 1, 4, 0, 3},  {F_LM, 7, 7, 51, 3, 1},
    {F_PC, 10, 1, 54, 13, 3}, {F_PC, 10, 3, 32, 5, 1},
    {F_PC, 14, 5, 24, 0, 1},  {F_OFF, 13, 4, 4, 0, 2},
    {F_TAG, 8, 4, 47, 11, 2}, {F_TAG, 2, 24, 32, 0, 1},
    {F_PC, 12, 10, 30, 0, 1}, {F_PC, 12, 9, 28, 0, 2},
    {F_PC, 12, 5, 31, 2, 2},  {F_TAG, 8, 10, 8, 7, 1},
    // # 23.392479
};
...
void sdbp_sampler::access(uint32_t tid, int set, int real_set, uint64_t tag,
                          uint64_t PC, int accessType, uint64_t paddr)
{
  ...
  if (i == params->dan_sampler_assoc) {
    ...
    if (i == params->dan_sampler_assoc) {
      ...
      for (j = 0; j < params->dan_sampler_assoc; j++) {
        if (blocks[j].lru_stack_position ==
            (unsigned int)(params->dan_sampler_assoc - 1)) {
          break;
        }
      }
    ...
    pred->block_is_dead(tid, &blocks[i], blocks[i].trace_buffer, true,
                        blocks[i].conf, params->dan_sampler_assoc);
  ...
  // record the trace
  make_trace(tid, pred, real_set, PC, tag, accessType, position == 0, is_fill,
             lastmiss_bits[real_set], paddr & 63);
  memcpy(blocks[i].trace_buffer, trace_buffer,
         (MAX_PATH_LENGTH + 1) * sizeof(unsigned int));
...
void sdbp_sampler::make_trace(uint32_t tid, perceptron_predictor *pred,
                              uint32_t setIndex, uint64_t PC, uint32_t tag,
                              int accessType, bool burst, bool insertion,
                              bool lastmiss, unsigned int offset)
{
  for (int i = 0; i < params->dan_predictor_tables; i++) {
    ...
    int           end_mask    = (1 << (f->end - begin_shift)) -
                   1;  // TODO: make this a hash instead of a simple bit extract
    switch (f->type) {
    case F_PC:
      if (f->which == 0) {
        ...
        trace_buffer[i] =
            (addresses[tid][f->which - 1] >> begin_shift) & end_mask;
      }
      break;
    case F_TAG:
      trace_buffer[i] =
          (((tag << lognsets6) | (setIndex << 6)) >> begin_shift) & end_mask;
      break;
    case F_BIAS:
      trace_buffer[i] = 0;
      break;
...
sdbp_sampler::sdbp_sampler(mp3b_params *_params, int nsets, int assoc)
    : params(_params)
{
  if (NUM_CPUS == 1) {
...
perceptron_predictor::perceptron_predictor(mp3b_params  *_params,
                                           sdbp_sampler *samp)
    : params(_params), parent(samp)
{
  // make the tables
...
void perceptron_predictor::block_is_dead(uint32_t            tid,
                                         sdbp_sampler_entry *block,
                                         unsigned int *trace_buffer, bool d,
                                         int conf, int pos)
{
