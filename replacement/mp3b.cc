...
void MP3BRepl::print_config()
{
  cout << "mp3b.promotion_threshold " << params->dan_promotion_threshold << endl
       ...
       << "mp3b.predictor_index_bits " << params->dan_predictor_index_bits
       << endl
...
void MP3BRepl::initialize_replacement()
{
  ...
  int sampler_set_bits =
      params->dan_sampler_assoc *
...
void MP3BRepl::update_replacement_state(uint32_t cpu, uint32_t set,
                                        uint32_t way, uint64_t paddr,
...
uint32_t MP3BRepl::find_victim(uint32_t cpu, uint64_t instr_id, uint32_t set,
...
void MP3BRepl::update_plru_mdpp(int set, int32_t wayID, bool hit, int *vector,
                                uint32_t accessType, bool really, int conf)
{
...
void MP3BRepl::UpdateSampler(uint32_t setIndex, uint64_t tag, uint32_t tid,
                             uint64_t PC, int32_t way, bool hit,
                             uint32_t accessType, uint64_t paddr)
{
  ...
  // determine if this is a sampler set
  {
    ...
    static unsigned int le13[] = {0x5c5,  0xcc5, 0xb6b,  0x1bc5, 0x8b,
                                  0x1782, 0x190, 0x15dd, 0x1af8, 0x75e,
                                  0x4a1,  0xb4b, 0x1196};
    ...
    // make the trace
    samp->make_trace(tid, samp->pred, setIndex, PC, tag, accessType, was_burst,
                     !hit, samp->lastmiss_bits[setIndex], paddr & 63);
...
stuff:
  ...
  if (record) {
    memmove(&samp->addresses[tid][1], &samp->addresses[tid][0],
...
int MP3BRepl::Get_Sampler_Victim(uint32_t tid, uint32_t setIndex,
                                 const BLOCK *current_set, uint32_t assoc,
                                 ...
                                 uint32_t accessType)
{
  ...
  if (accessType != WRITEBACK) {
    ...
    samp->make_trace(tid, samp->pred, setIndex, PC, tag, accessType, false,
                     true, samp->lastmiss_bits[setIndex], paddr & 63);
    int prediction;
    int conf = samp->pred->get_prediction(tid, setIndex);
    if (params->dan_dt2) {
      if (setIndex < samp->leaders1) {
